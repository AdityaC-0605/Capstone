...
class CSVDataSource(DataSource):
    ...
    def read_data(self, source_path: str, **kwargs) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(f"Successfully read CSV file: {source_path}, shape: {data.shape}")
            return data
...
class JSONDataSource(DataSource):
    ...
    def read_data(self, source_path: str, **kwargs) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(f"Successfully read JSON file: {source_path}, shape: {data.shape}")
            return data
...
class ParquetDataSource(DataSource):
    ...
    def read_data(self, source_path: str, **kwargs) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(f"Successfully read Parquet file: {source_path}, shape: {data.shape}")
            return data
...
class BankingDataValidator:
    ...
    def __init__(self):
        self.required_columns = [
            'customer_id', 'age', 'annual_income_inr', 'loan_amount_inr',
            'credit_score', 'debt_to_income_ratio', 'default'
        ]
        ...
        self.numeric_columns = [
            'age', 'annual_income_inr', 'loan_amount_inr', 'loan_term_months',
            'credit_score', 'num_open_credit_accounts', 'num_delinquent_accounts',
            'debt_to_income_ratio', 'past_default', 'default'
        ]
        ...
        self.categorical_rules = {
            ...
            'employment_type': ['Salaried', 'Self-Employed', 'Government', 'Unemployed'],
            'loan_purpose': ['Personal Loan', 'Home Loan', 'Education Loan', 'Credit Card', 'Business Loan']
        }
    ...
    def _validate_data_types(self, data: pd.DataFrame) -> List[ValidationIssue]:
        ...
        for column in self.numeric_columns:
            if column in data.columns:
                non_numeric = data[column].apply(lambda x: not pd.api.types.is_numeric_dtype(type(x)) and pd.notna(x))
                if non_numeric.any():
    ...
    def _validate_missing_values(self, data: pd.DataFrame) -> List[ValidationIssue]:
        ...
        for column in data.columns:
            ...
            if missing_count > 0:
                ...
                severity = ValidationSeverity.CRITICAL if column in self.required_columns else ValidationSeverity.WARNING
    ...
    def _validate_value_ranges(self, data: pd.DataFrame) -> List[ValidationIssue]:
        ...
        for column, rules in self.validation_rules.items():
            ...
            if 'min' in rules and 'max' in rules:
                out_of_range = (data[column] < rules['min']) | (data[column] > rules['max'])
                ...
                invalid_values = ~data[column].isin(rules['values']) & data[column].notna()
                ...
                if invalid_values.any():
                    ...
                    issues.append(ValidationIssue(
                        ...
                        count=count,
                        percentage=percentage,
                        sample_values=data[column][invalid_values].unique().tolist()[:5]
                    ))
    ...
    def _validate_business_logic(self, data: pd.DataFrame) -> List[ValidationIssue]:
        ...
        if all(col in data.columns for col in ['loan_amount_inr', 'annual_income_inr']):
            ...
            unreasonable_ratio = loan_to_income_ratio > 10  # More than 10x annual income
        ...
        if all(col in data.columns for col in ['debt_to_income_ratio', 'loan_amount_inr', 'annual_income_inr']):
...
class DataProfiler:
    ...
    def profile_data(self, data: pd.DataFrame) -> DataProfile:
        ...
        missing_data_percentage = (data.isnull().sum().sum() / (total_rows * total_columns)) * 100
        duplicate_rows = data.duplicated().sum()
    ...
    def _calculate_quality_score(self, data: pd.DataFrame, column_profiles: Dict[str, Dict[str, Any]]) -> float:
...
class DataIngestionProcessor(DataProcessor):
    ...
    def process(self, source_path: str, data_format: Optional[DataFormat] = None, 
                validate: bool = True, profile: bool = True, **kwargs) -> IngestionResult:
        ...
        try:
            ...
            return IngestionResult(
                ...
                message="Data ingestion completed successfully" if success else f"Data ingestion failed with {len(critical_issues)} critical issues"
            )
    ...
    def generate_validation_report(self, validation_issues: List[ValidationIssue]) -> Dict[str, Any]:
    ...
    def generate_data_profile_report(self, data_profile: DataProfile) -> Dict[str, Any]:
        ...
        return {
            'overview': {
                ...
                'missing_data_percentage': round(data_profile.missing_data_percentage, 2),
            ...
            'column_profiles': {
                column: {
                    **profile,
                    'missing_percentage': round(profile.get('missing_percentage', 0), 2),
