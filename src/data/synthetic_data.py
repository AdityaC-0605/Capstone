...
class CTGANWrapper:
    ...
    def __init__(self, config: SyntheticDataConfig):
        ...
        try:
            ...
            logger.warning("CTGAN not available. Using fallback synthetic data generation.")
    ...
    def _postprocess_data(self, synthetic_data: pd.DataFrame) -> pd.DataFrame:
        ...
        for column in self.discrete_columns:
            if column in self.label_encoders:
                ...
                processed_data[column] = np.clip(processed_data[column].round(), 0, max_label)
...
class SyntheticDataEvaluator:
    ...
    def evaluate_quality(self, original_data: pd.DataFrame, 
                        synthetic_data: pd.DataFrame) -> SyntheticDataQuality:
        ...
        try:
            ...
            if self.config.evaluate_ml_efficacy:
                quality.ml_efficacy_score, quality.feature_importance_similarity = self._evaluate_ml_efficacy(
                    original_data, synthetic_data
                )
            
            # Privacy evaluation
            if self.config.privacy_test_enabled:
                quality.privacy_score, quality.nearest_neighbor_distance = self._evaluate_privacy(
                    original_data, synthetic_data
                )
            ...
            logger.info(f"Synthetic data quality evaluation completed. Overall score: {quality.overall_quality_score:.3f}")
    ...
    def _evaluate_column_distributions(self, original: pd.DataFrame, 
    ...
    def _evaluate_wasserstein_distances(self, original: pd.DataFrame, 
                                      synthetic: pd.DataFrame) -> Dict[str, float]:
    ...
    def _evaluate_correlation_similarity(self, original: pd.DataFrame, 
                                       synthetic: pd.DataFrame) -> float:
        """Evaluate correlation matrix similarity."""
        try:
            # Get numeric columns
            numeric_columns = original.select_dtypes(include=[np.number]).columns
            common_numeric = [col for col in numeric_columns if col in synthetic.columns]
            ...
            if len(orig_corr_flat) > 0:
                correlation_similarity = np.corrcoef(orig_corr_flat, synth_corr_flat)[0, 1]
                return max(0, correlation_similarity) if not np.isnan(correlation_similarity) else 0.0
            else:
                return 1.0
    ...
    def _evaluate_ml_efficacy(self, original: pd.DataFrame, 
                            synthetic: pd.DataFrame) -> Tuple[float, float]:
        ...
        try:
            ...
            if hasattr(rf_orig, 'feature_importances_') and hasattr(rf_synth, 'feature_importances_'):
                importance_corr = np.corrcoef(
                    rf_orig.feature_importances_,
                    rf_synth.feature_importances_
                )[0, 1]
                feature_importance_similarity = max(0, importance_corr) if not np.isnan(importance_corr) else 0.0
            else:
    ...
    def _evaluate_privacy(self, original: pd.DataFrame, 
                         synthetic: pd.DataFrame) -> Tuple[float, float]:
        ...
        try:
            ...
            # Align columns
            common_cols = [col for col in orig_encoded.columns if col in synth_encoded.columns]
...
class SyntheticDataGenerator(DataProcessor):
    ...
    def process(self, data: pd.DataFrame, target_column: Optional[str] = None) -> SyntheticDataResult:
        ...
        try:
            ...
            logger.info(f"Synthetic data generation completed in {processing_time:.2f} seconds")
    ...
    def _validate_quality_thresholds(self, quality_metrics: SyntheticDataQuality) -> bool:
        ...
        if quality_metrics.overall_quality_score < min_overall_quality:
            logger.warning(f"Overall quality score {quality_metrics.overall_quality_score:.3f} below threshold {min_overall_quality}")
            return False
        
        if quality_metrics.ml_efficacy_score < min_ml_efficacy:
            logger.warning(f"ML efficacy score {quality_metrics.ml_efficacy_score:.3f} below threshold {min_ml_efficacy}")
            return False
        
        if quality_metrics.correlation_similarity < min_correlation_similarity:
            logger.warning(f"Correlation similarity {quality_metrics.correlation_similarity:.3f} below threshold {min_correlation_similarity}")
            return False
        
        return True
    
    def generate_quality_report(self, quality_metrics: SyntheticDataQuality) -> Dict[str, Any]:
        ...
        return {
            ...
            'statistical_similarity': {
                ...
                'average_ks_score': np.mean(list(quality_metrics.kolmogorov_smirnov_scores.values())) if quality_metrics.kolmogorov_smirnov_scores else 0.0,
                'average_wasserstein_score': np.mean(list(quality_metrics.wasserstein_distances.values())) if quality_metrics.wasserstein_distances else 0.0
            },
...
def create_synthetic_data_generator(config: Optional[SyntheticDataConfig] = None) -> SyntheticDataGenerator:
    """Create a synthetic data generator instance."""
    return SyntheticDataGenerator(config)


def generate_synthetic_banking_data(data: pd.DataFrame, 
                                   target_column: str = 'default',
                                   config: Optional[SyntheticDataConfig] = None) -> SyntheticDataResult:
