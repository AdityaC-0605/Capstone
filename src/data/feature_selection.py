...
class FeatureSelectionConfig:
    ...
    importance_methods: List[str] = field(default_factory=lambda: ["mutual_info", "correlation", "model_based"])
...
class StatisticalFeatureSelector:
    ...
    def __init__(self, config: FeatureSelectionConfig):
        ...
        self.selectors = {
            ...
            'mutual_info': SelectKBest(mutual_info_classif, k=config.statistical_k_features)
        }
    
    def select_features(self, X: pd.DataFrame, y: pd.Series) -> Tuple[List[str], Dict[str, float]]:
        ...
        try:
            ...
            logger.info(f"Statistical selection ({method}): {len(selected_features)}/{len(X.columns)} features selected")
...
class CorrelationAnalyzer:
    ...
    def analyze_correlations(self, X: pd.DataFrame, y: pd.Series) -> Tuple[pd.DataFrame, List[Tuple[str, str, float]], Dict[str, float]]:
        ...
        logger.info(f"Found {len(multicollinear_pairs)} highly correlated feature pairs")
        
        return corr_matrix, multicollinear_pairs, target_correlations
    
    def remove_multicollinear_features(self, X: pd.DataFrame, y: pd.Series, 
                                     multicollinear_pairs: List[Tuple[str, str, float]]) -> Tuple[pd.DataFrame, Dict[str, str]]:
        ...
        for feature1, feature2, corr_value in multicollinear_pairs:
            ...
            if corr1 >= corr2:
                ...
                removed_features[feature2] = f"Multicollinear with {feature1} (corr={corr_value:.3f})"
            else:
                features_to_remove.add(feature1)
                removed_features[feature1] = f"Multicollinear with {feature2} (corr={corr_value:.3f})"
...
class RecursiveFeatureEliminator:
    ...
    def select_features(self, X: pd.DataFrame, y: pd.Series) -> Tuple[List[str], Dict[str, int]]:
        ...
        try:
            ...
            logger.info(f"RFE selection: {len(selected_features)}/{len(X.columns)} features selected")
...
class ModelBasedFeatureSelector:
    ...
    def __init__(self, config: FeatureSelectionConfig):
        ...
        self.estimators = {
            ...
            'logistic_regression': LogisticRegression(random_state=42, max_iter=1000, penalty='l1', solver='liblinear')
        }
    ...
    def select_features(self, X: pd.DataFrame, y: pd.Series) -> Tuple[List[str], Dict[str, float]]:
        ...
        if estimator is None:
            logger.warning(f"Unknown model-based estimator: {self.config.model_based_estimator}")
            return list(X.columns), {}
        ...
        try:
            ...
            logger.info(f"Model-based selection: {len(selected_features)}/{len(X.columns)} features selected")
...
class FeatureSelectionPipeline(DataProcessor):
    ...
    def process(self, X: pd.DataFrame, y: pd.Series) -> FeatureSelectionResult:
        ...
        try:
            ...
            # Initialize tracking
            all_feature_scores = {feature: FeatureImportanceScore(feature_name=feature) 
            ...
            if self.config.enable_variance_threshold:
                X_current, low_var_removed = self._remove_low_variance_features(X_current)
                removed_features.update(low_var_removed)
            
            # 2. Correlation analysis and multicollinearity removal
            corr_matrix, multicollinear_pairs, target_correlations = self.correlation_analyzer.analyze_correlations(X_current, y)
            
            if multicollinear_pairs:
                X_current, multicoll_removed = self.correlation_analyzer.remove_multicollinear_features(X_current, y, multicollinear_pairs)
            ...
            statistical_features, statistical_scores = self.statistical_selector.select_features(X_current, y)
            ...
            model_features, model_importance = self.model_selector.select_features(X_current, y)
            ...
            feature_scores_list = self._calculate_combined_scores(all_feature_scores, selected_features)
            
            # Create selection summary
            selection_summary = {
                'original_features': X.shape[1],
                'after_variance_threshold': X_current.shape[1] if self.config.enable_variance_threshold else X.shape[1],
    ...
    def _remove_low_variance_features(self, X: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, str]]:
        ...
        if len(numeric_features.columns) > 0:
            ...
            for col in removed_cols:
                removed_features[col] = f"Low variance (< {self.config.variance_threshold})"
    ...
    def _combine_selection_results(self, all_features: List[str], 
                                 statistical_features: List[str],
                                 rfe_features: List[str],
                                 model_features: List[str],
                                 feature_scores: Dict[str, FeatureImportanceScore]) -> List[str]:
        ...
        selected_by_voting = [feature for feature, votes in feature_votes.items() if votes >= 2]
        ...
        if len(selected_by_voting) < self.config.top_k_features:
            ...
            for feature in all_features:
                if feature in feature_scores:
                    ...
                    combined = (score_obj.mutual_info_score + 
                              score_obj.correlation_score + 
                              score_obj.model_importance_score) / 3
            ...
            for feature, _ in temp_scores:
                if feature not in selected_by_voting and len(selected_by_voting) < self.config.top_k_features:
                    selected_by_voting.append(feature)
        
        return selected_by_voting[:self.config.top_k_features]
    
    def _calculate_combined_scores(self, feature_scores: Dict[str, FeatureImportanceScore], 
                                 selected_features: List[str]) -> List[FeatureImportanceScore]:
        ...
        all_model_importance = [score.model_importance_score for score in feature_scores.values()]
        ...
        max_model_importance = max(all_model_importance) if max(all_model_importance) > 0 else 1
        ...
        for feature_name, score_obj in feature_scores.items():
            ...
            normalized_model_importance = score_obj.model_importance_score / max_model_importance
            
            # Weighted combination (can be adjusted)
            combined_score = (0.4 * normalized_mutual_info + 
...
def create_feature_selection_pipeline(config: Optional[FeatureSelectionConfig] = None) -> FeatureSelectionPipeline:
    """Create a feature selection pipeline instance."""
    return FeatureSelectionPipeline(config)


def select_banking_features(X: pd.DataFrame, y: pd.Series, 
