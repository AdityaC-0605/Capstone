...
try:
    ...
    from ..sustainability.sustainability_monitor import SustainabilityMonitor, track_sustainability
except ImportError:
...
class CreditApplication(BaseModel):
    ...
    def validate_loan_purpose(cls, v):
        valid_purposes = ['debt_consolidation', 'home_improvement', 'major_purchase', 
                         'medical', 'vacation', 'wedding', 'moving', 'other']
...
class PredictionRequest(BaseModel):
    ...
    explanation_type: str = Field("shap", description="Type of explanation (shap, lime)")
...
class InferenceService:
    ...
    def __init__(self, config: Optional[APIConfig] = None):
        if not FASTAPI_AVAILABLE:
            raise ImportError("FastAPI is required for inference service. "
    ...
    def _setup_routes(self):
        ...
        async def api_key_info(api_key: str = Depends(self._verify_api_key)):
            ...
            return {
                ...
                "last_used": usage_info.get("last_used", "Never").isoformat() if isinstance(usage_info.get("last_used"), datetime) else "Never"
            }
    ...
    async def _make_prediction(self, request: PredictionRequest, 
                             background_tasks: BackgroundTasks, api_key: str) -> PredictionResponse:
        ...
        try:
            ...
            if self.config.enable_sustainability_tracking and request.track_sustainability:
                exp_id = f"prediction_{prediction_id}"
                self.sustainability_monitor.start_experiment_tracking(exp_id, {
            ...
            if request.include_explanation and self.explainer:
                explanation = self.explainer.explain_prediction(input_data, prediction_result)
            
            # Stop sustainability tracking
            sustainability_metrics = None
            if sustainability_context:
                sustainability_metrics = self.sustainability_monitor.stop_experiment_tracking(sustainability_context)
            ...
            if self.config.log_predictions:
                background_tasks.add_task(self._log_prediction, request, response, api_key)
            ...
            if sustainability_context:
                try:
                    self.sustainability_monitor.stop_experiment_tracking(sustainability_context)
                except:
                    pass
    ...
    async def _make_batch_prediction(self, request: BatchPredictionRequest,
                                   background_tasks: BackgroundTasks, api_key: str) -> BatchPredictionResponse:
        ...
        try:
            ...
            for i, application in enumerate(request.applications):
                try:
                    ...
                    pred_response = await self._make_single_prediction_internal(individual_request)
            ...
            batch_summary = {
                ...
                "risk_distribution": self._calculate_risk_distribution(successful_predictions)
            }
            ...
            if self.config.log_predictions:
                background_tasks.add_task(self._log_batch_prediction, request, response, api_key)
    ...
    async def _make_single_prediction_internal(self, request: PredictionRequest) -> PredictionResponse:
    ...
    def _calculate_risk_distribution(self, predictions: List[PredictionResponse]) -> Dict[str, int]:
    ...
    async def _log_prediction(self, request: PredictionRequest, response: PredictionResponse, api_key: str):
    ...
    async def _log_batch_prediction(self, request: BatchPredictionRequest, 
                                  response: BatchPredictionResponse, api_key: str):
...
def run_inference_service(host: str = "0.0.0.0", port: int = 8000, 
