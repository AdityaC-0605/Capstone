...
class InputSanitizer:
        ...
        for field_name, value in data.items():
            if field_name in self.sanitization_rules:
                for rule in self.sanitization_rules[field_name]:
                    try:
                        sanitized_value, warning = self._apply_sanitization_rule(value, rule)
            ...
            if isinstance(value, str):
                sanitized_value, general_warnings = self._general_string_sanitization(value)
                sanitized_data[field_name] = sanitized_value
                warnings.extend([f"{field_name}: {w}" for w in general_warnings])
        
        return sanitized_data, warnings
    
    def _apply_sanitization_rule(self, value: Any, rule: SanitizationRule) -> Tuple[Any, Optional[str]]:
        ...
        if rule.sanitizer_type == "strip":
            ...
            if isinstance(value, str):
                ...
                return normalized, "Whitespace normalized" if normalized != value else None
    ...
    def _check_encoding_attacks(self, value: str) -> bool:
        ...
        encoding_count = sum(1 for pattern in encoded_patterns 
...
class InputValidator:
    ...
    def validate_input(self, data: Dict[str, Any], schema_name: Optional[str] = None) -> Tuple[bool, List[str]]:
    ...
    def _apply_validation_rule(self, value: Any, rule: InputValidationRule) -> List[str]:
...
class AnomalyDetector:
    ...
    def detect_anomalies(self, request_data: Dict[str, Any], 
                        client_id: str = "unknown") -> List[Tuple[InputAnomalyType, str, float]]:
        ...
        for field_name, value in request_data.items():
            if isinstance(value, (int, float)):
                ...
                if len(stats["values"]) > 1:
                    ...
                    stats["mean"] = np.mean(values) if NUMPY_AVAILABLE else sum(values) / len(values)
                    if NUMPY_AVAILABLE:
    ...
    def _update_client_patterns(self, client_id: str, request_info: Dict[str, Any]):
        ...
        if pattern["last_request"]:
            interval = (request_info["timestamp"] - pattern["last_request"]).total_seconds()
    ...
    def _detect_statistical_outliers(self, request_data: Dict[str, Any]) -> List[Tuple[InputAnomalyType, str, float]]:
    ...
    def _detect_pattern_anomalies(self, request_data: Dict[str, Any], 
                                 client_id: str) -> List[Tuple[InputAnomalyType, str, float]]:
    ...
    def _detect_frequency_anomalies(self, client_id: str) -> List[Tuple[InputAnomalyType, str, float]]:
    ...
    def _detect_size_anomalies(self, request_data: Dict[str, Any]) -> List[Tuple[InputAnomalyType, str, float]]:
        ...
        if len(self.request_history) > 10:
            historical_sizes = [req["size"] for req in list(self.request_history)[-100:]]
...
class DeadLetterQueue:
    ...
    def add_message(self, request_data: Dict[str, Any], error_type: ErrorType, 
                   error_message: str, max_retries: int = 3) -> str:
        ...
        logger.warning(f"Added message to dead letter queue: {message_id} - {error_message}")
        ...
        if message_id in self.queue:
            ...
            if success:
                ...
                if message.retry_count >= message.max_retries:
                    logger.error(f"Dead letter message {message_id} exceeded max retries")
                else:
                    # Exponential backoff
                    delay_minutes = 2 ** message.retry_count
                    message.next_retry_at = datetime.now() + timedelta(minutes=delay_minutes)
                    logger.warning(f"Dead letter message {message_id} scheduled for retry in {delay_minutes} minutes")
    ...
    def get_queue_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "oldest_message": min(self.queue.values(), key=lambda m: m.timestamp).timestamp.isoformat() if self.queue else None
        }
...
class ErrorLogger:
    ...
    def __init__(self, max_history: int = 10000):
        ...
        self.client_error_counts: Dict[str, Dict[ErrorType, int]] = defaultdict(lambda: defaultdict(int))
        
    def log_error(self, error_type: ErrorType, message: str, 
                 details: Dict[str, Any] = None, client_id: str = None,
                 endpoint: str = None, request_id: str = None,
    ...
    def _get_top_error_clients(self, errors: List[ErrorEvent], top_n: int = 10) -> List[Dict[str, Any]]:
...
class ErrorHandler:
    ...
    def _setup_default_rules(self):
        ...
        self.validator.add_validation_rule(InputValidationRule(
        ...
        for field in ["loan_purpose", "home_ownership", "verification_status"]:
            self.sanitizer.add_sanitization_rule(SanitizationRule(
                field_name=field,
                sanitizer_type="normalize",
                parameters={}
            ))
    ...
    async def process_request(self, request_data: Dict[str, Any], 
                            ...
                            endpoint: str = "unknown") -> Tuple[bool, Dict[str, Any], List[str]]:
        ...
        try:
            ...
            sanitized_data, sanitization_warnings = self.sanitizer.sanitize_input(request_data)
            warnings.extend(sanitization_warnings)
            
            # 2. Adversarial pattern detection
            adversarial_threats = self.sanitizer.detect_adversarial_patterns(sanitized_data)
            if adversarial_threats:
                error_id = self.error_logger.log_error(
                    ...
                    client_id, endpoint, request_id,
                    SeverityLevel.HIGH
                )
                
                self.dead_letter_queue.add_message(
                    request_data, ErrorType.ADVERSARIAL_INPUT,
                    f"Adversarial input: {'; '.join(adversarial_threats)}"
                )
                
                return False, {}, [f"Request blocked due to adversarial input (Error ID: {error_id})"]
            ...
            if not is_valid:
                error_id = self.error_logger.log_error(
                    ...
                    {"validation_errors": validation_errors, "sanitized_data": sanitized_data},
                    client_id, endpoint, request_id,
                    SeverityLevel.MEDIUM
                )
                
                self.dead_letter_queue.add_message(
                    request_data, ErrorType.VALIDATION_ERROR,
            ...
            anomalies = self.anomaly_detector.detect_anomalies(sanitized_data, client_id)
            
            if anomalies:
                high_risk_anomalies = [a for a in anomalies if a[2] > 2.0]  # High confidence anomalies
                
                if high_risk_anomalies:
                    anomaly_descriptions = [f"{a[0].value}: {a[1]}" for a in high_risk_anomalies]
                    
                    error_id = self.error_logger.log_error(
                        ErrorType.ANOMALY_DETECTED,
                        f"High-risk anomalies detected: {'; '.join(anomaly_descriptions)}",
                        {"anomalies": [{"type": a[0].value, "description": a[1], "score": a[2]} for a in anomalies]},
                        client_id, endpoint, request_id,
                        SeverityLevel.HIGH
                    )
                    
                    return False, {}, [f"Request blocked due to anomalies (Error ID: {error_id})"]
                
                # Log low-risk anomalies as warnings
                for anomaly in anomalies:
                    warnings.append(f"Anomaly detected - {anomaly[0].value}: {anomaly[1]} (score: {anomaly[2]:.2f})")
            ...
            self.dead_letter_queue.add_message(
                request_data, ErrorType.SYSTEM_ERROR,
                f"System error: {str(e)}"
            )
            
            return False, {}, [f"Internal error processing request (Error ID: {error_id})"]
    ...
    async def process_dead_letter_retries(self):
        ...
        for message in candidates:
            try:
                ...
                if success:
                    logger.info(f"Successfully reprocessed dead letter message {message.id}")
                
            except Exception as e:
                logger.error(f"Failed to reprocess dead letter message {message.id}: {e}")
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        success, data, warnings = await handler.process_request(normal_request, "test_client", "/predict")
        ...
        success, data, warnings = await handler.process_request(malicious_request, "malicious_client", "/predict")
