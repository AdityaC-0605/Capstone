...
class ModelComparison:
    ...
    def compare_models(self, X: pd.DataFrame, y: pd.Series) -> List[ModelBenchmarkResult]:
    ...
    def _benchmark_lightgbm(self, X: pd.DataFrame, y: pd.Series) -> ModelBenchmarkResult:
        ...
        try:
            ...
            lgb_result = trainer.train_and_evaluate(X, y, test_size=self.config.test_size)
            ...
            if lgb_result.success:
                ...
                result.num_parameters = self._estimate_lightgbm_parameters(lgb_result.model)
    ...
    def _benchmark_dnn(self, X: pd.DataFrame, y: pd.Series) -> ModelBenchmarkResult:
        ...
        try:
            ...
            dnn_result = trainer.train_and_evaluate(X, y, test_size=self.config.test_size)
            ...
            if dnn_result.success:
                ...
                result.num_parameters = sum(p.numel() for p in dnn_result.model.parameters())
    ...
    def _estimate_lightgbm_size(self, model) -> float:
        ...
        try:
            if model and model.is_trained:
                ...
                estimated_bytes = num_trees * 31 * 8 * num_features  # 31 is default num_leaves
    ...
    def _generate_comparison_report(self) -> str:
        ...
        for result in self.results:
            if result.success:
                report.append(f"{result.model_name:<20} {result.auc_roc:<8.4f} "
        ...
        report.append("-" * 50)
        report.append(f"{'Model':<20} {'Train Time':<12} {'Inference':<12} {'Size (MB)':<10}")
        report.append("-" * 50)
        ...
        for result in self.results:
            if result.success:
                ...
                report.append(f"{result.model_name:<20} {train_time:<12} "
        ...
        for result in self.results:
            if result.success:
                ...
                report.append(f"{result.model_name:<20} {params:<12} {result.model_type:<15}")
        ...
        if successful_results:
            ...
            fastest_train = min(successful_results, key=lambda r: r.training_time_seconds)
            fastest_inference = min(successful_results, key=lambda r: r.inference_time_ms)
            smallest = min(successful_results, key=lambda r: r.model_size_mb)
            
            report.append(f"Best AUC-ROC: {best_auc.model_name} ({best_auc.auc_roc:.4f})")
            report.append(f"Fastest Training: {fastest_train.model_name} ({fastest_train.training_time_seconds:.2f}s)")
            report.append(f"Fastest Inference: {fastest_inference.model_name} ({fastest_inference.inference_time_ms:.2f}ms)")
            report.append(f"Smallest Model: {smallest.model_name} ({smallest.model_size_mb:.2f} MB)")
        ...
        if successful_results:
            ...
            best_overall = max(successful_results, 
                             key=lambda r: r.auc_roc * 0.4 + (1/max(r.training_time_seconds, 0.1)) * 0.3 + 
            ...
            if len(successful_results) > 1:
                lgb_results = [r for r in successful_results if r.model_type == 'gradient_boosting']
                dnn_results = [r for r in successful_results if r.model_type == 'deep_learning']
                ...
                if lgb_results and dnn_results:
                    ...
                    if lgb.training_time_seconds < dnn.training_time_seconds:
                        ...
                        report.append(f"  (Faster training: {lgb.training_time_seconds:.2f}s vs {dnn.training_time_seconds:.2f}s)")
                    
                    if dnn.auc_roc > lgb.auc_roc:
                        report.append(f"• For maximum accuracy: {dnn.model_name}")
                        report.append(f"  (Higher AUC: {dnn.auc_roc:.4f} vs {lgb.auc_roc:.4f})")
        
        return "\n".join(report)


# Convenience functions
def compare_lightgbm_vs_dnn(X: pd.DataFrame, y: pd.Series, 
                           config: Optional[ComparisonConfig] = None) -> List[ModelBenchmarkResult]:
