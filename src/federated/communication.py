...
class MessageSerializer:
    ...
    def serialize_model_weights(weights: Dict[str, torch.Tensor]) -> str:
        ...
        try:
            ...
            logger.debug(f"Serialized model weights: {len(weights)} tensors, "
...
class FederatedCommunicationManager:
    ...
    def __init__(self, config: FederatedConfig, secure_communicator: Optional[SecureCommunicator] = None):
        self.config = config
    ...
    async def send_message(self, message: FederatedMessage, target_url: str) -> Optional[FederatedMessage]:
        """
        ...
        try:
            ...
            async with self.session.post(target_url, data=serialized_message, headers=headers) as response:
                if response.status == 200:
                    response_data = await response.text()
                    if response_data:
                        response_message = MessageSerializer.deserialize_message(response_data)
                        
                        # Verify signature if present
                        if response_message.signature and response_message.sender_id:
                            if not self.communicator.verify_signature(
                                response_data, response_message.signature, response_message.sender_id
                            ):
                                logger.warning(f"Invalid signature in response from {response_message.sender_id}")
                                return None
                        
                        # Decrypt if encrypted
                        if response_message.encrypted:
                            encrypted_data = response_message.payload.get("encrypted_data")
                            if encrypted_data:
                                decrypted_data = self.communicator.decrypt_message(
                                    encrypted_data, response_message.sender_id
                                )
                                response_message = MessageSerializer.deserialize_message(decrypted_data)
                        
                        return response_message
                else:
                    logger.error(f"HTTP error {response.status} when sending message to {target_url}")
    ...
    async def handle_incoming_message(self, request_data: str, sender_ip: str) -> Optional[FederatedMessage]:
        """
        ...
        try:
            ...
            if message.signature and message.sender_id:
                if not self.communicator.verify_signature(
                    ...
                    logger.warning(f"Invalid signature from {message.sender_id} at {sender_ip}")
    ...
    def _create_error_response(self, original_message: FederatedMessage, error_message: str) -> FederatedMessage:
        ...
        return FederatedMessage(
            ...
            payload={"error": error_message, "original_message_id": original_message.message_id}
        )
    
    def create_model_update_message(self, sender_id: str, recipient_id: str, 
                                  model_update: ModelUpdate) -> FederatedMessage:
        """
        ...
        serialized_weights = MessageSerializer.serialize_model_weights(model_update.model_weights)
    ...
    def parse_model_update_message(self, message: FederatedMessage) -> ModelUpdate:
        ...
        model_weights = MessageSerializer.deserialize_model_weights(payload["model_weights"])
        ...
        return ModelUpdate(
            ...
            differential_privacy_applied=payload.get("differential_privacy_applied", False),
...
# Utility functions
def create_registration_message(client_id: str, public_key: str, 
                               ip_address: str, port: int,
                               capabilities: Optional[Dict[str, Any]] = None) -> FederatedMessage:
