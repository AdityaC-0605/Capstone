...
try:
    from .federated_server import FederatedServer, FederatedConfig, ModelUpdate, ClientInfo
    from .communication import (
        FederatedCommunicationManager, MessageType, FederatedMessage,
        MessageSerializer, create_registration_message
    )
    ...
    from federated.federated_server import FederatedServer, FederatedConfig, ModelUpdate, ClientInfo
...
class ClientRegistrationRequest(BaseModel):
    ...
    capabilities: Optional[Dict[str, Any]] = Field(default={}, description="Client capabilities")
    data_size: Optional[int] = Field(default=0, description="Size of client's training data")
...
class FederatedServerAPI:
    ...
    def __init__(self, federated_server: FederatedServer, config: Optional[FederatedConfig] = None):
    ...
    def _setup_routes(self):
            ...
            try:
                ...
                if success:
                    ...
                    response = ClientRegistrationResponse(
                        ...
                        authentication_token=client_info.authentication_token if client_info else None,
                        server_public_key=self.communication_manager.communicator.get_public_key_pem(),
                        client_id=request.client_id
                    )
                    
                    logger.info(f"Client {request.client_id} registered from {client_ip}")
                    return response
                else:
        ...
        async def get_global_model(
            ...
            try:
                ...
                if global_weights:
                    # Serialize weights
                    encoded_weights = MessageSerializer.serialize_model_weights(global_weights)
        ...
        async def submit_model_update(
            ...
            try:
                ...
                model_weights = MessageSerializer.deserialize_model_weights(request.model_weights_encoded)
                ...
                logger.error(f"Error processing model update from client {request.client_id}: {e}")
                raise HTTPException(status_code=500, detail=str(e))
    ...
    def _authenticate_client(self, client_id: str, credentials: Optional[HTTPAuthorizationCredentials]) -> bool:
    ...
    async def _process_model_update(self, model_update: ModelUpdate):
        ...
        try:
            ...
            logger.info(f"Processing model update from client {model_update.client_id} "
            ...
            logger.error(f"Error processing model update from {model_update.client_id}: {e}")
...
# Utility functions
def create_federated_server_api(config_dict: Optional[Dict[str, Any]] = None) -> FederatedServerAPI:
    """
...
async def run_federated_server_async(api: FederatedServerAPI, host: str = "0.0.0.0", port: int = 8080):
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--require-auth", action="store_true", help="Require client authentication")
    parser.add_argument("--enable-encryption", action="store_true", help="Enable message encryption")
