...
class CarbonOffsetConfig:
    ...
    # Project preferences
    preferred_project_types: List[OffsetProjectType] = field(default_factory=lambda: [
        OffsetProjectType.RENEWABLE_ENERGY,
        OffsetProjectType.FOREST_CONSERVATION,
        OffsetProjectType.REFORESTATION
    ])
    preferred_verification_standards: List[OffsetVerificationStandard] = field(default_factory=lambda: [
...
class CarbonOffsetMarketplace:
            ...
            try:
                ...
                for purchase_data in purchases_data:
                    purchase = CarbonOffsetPurchase(
                        ...
                        offset_certificate_id=purchase_data.get("offset_certificate_id"),
                        verification_status=purchase_data.get("verification_status", "pending"),
                        verification_date=datetime.fromisoformat(purchase_data["verification_date"]) if purchase_data.get("verification_date") else None,
                ...
                logger.info(f"Loaded {len(self.purchase_history)} carbon offset purchases")
    ...
    def get_available_projects(self, 
                             ...
                             min_available_credits: Optional[int] = None) -> List[CarbonOffsetProject]:
        ...
        if project_types:
            filtered_projects = [p for p in filtered_projects if p.project_type in project_types]
        
        # Filter by price
        if max_price_per_ton:
            filtered_projects = [p for p in filtered_projects if p.price_per_ton_usd <= max_price_per_ton]
        
        # Filter by available credits
        if min_available_credits:
            filtered_projects = [p for p in filtered_projects if p.available_credits >= min_available_credits]
    ...
    def calculate_required_offset(self, carbon_footprint: CarbonFootprint, 
                                offset_ratio: Optional[float] = None) -> float:
        ...
        required_offset_kg = float(Decimal(str(required_offset_kg)).quantize(Decimal('0.1'), rounding=ROUND_HALF_UP))
        
        return required_offset_kg
    
    def find_best_offset_project(self, required_offset_kg: float) -> Optional[CarbonOffsetProject]:
        """Find the best offset project for the required amount."""
        
        # Filter projects that can fulfill the requirement
        suitable_projects = [
            p for p in self.available_projects
            if (p.available_credits >= required_offset_kg / 1000 and  # Convert kg to tonnes
                p.min_purchase_kg <= required_offset_kg and
                p.project_type in self.config.preferred_project_types and
                p.verification_standard in self.config.preferred_verification_standards)
        ]
        
        if not suitable_projects:
            logger.warning(f"No suitable offset projects found for {required_offset_kg:.2f} kg")
            return None
    ...
    def purchase_carbon_offset(self, carbon_footprint: CarbonFootprint,
                             ai_experiment_id: Optional[str] = None,
                             offset_ratio: Optional[float] = None) -> Optional[CarbonOffsetPurchase]:
        ...
        required_offset_kg = self.calculate_required_offset(carbon_footprint, offset_ratio)
        
        # Check if offset is above threshold
        if required_offset_kg < self.config.auto_offset_threshold_kg:
            logger.info(f"Carbon footprint {required_offset_kg:.4f} kg below auto-offset threshold")
            return None
        ...
        if total_cost_usd > self.config.max_single_purchase_usd:
            logger.warning(f"Purchase cost ${total_cost_usd:.2f} exceeds single purchase limit")
            return None
        ...
        if success:
            ...
            logger.info(f"Successfully purchased {required_offset_kg:.2f} kg carbon offset for ${total_cost_usd:.2f}")
            return purchase
    ...
    def _process_purchase(self, purchase: CarbonOffsetPurchase, project: CarbonOffsetProject) -> bool:
        ...
        try:
            ...
            purchase.transaction_id = f"TXN_{int(time.time())}_{purchase.purchase_id[:8]}"
    ...
    def get_offset_summary(self, days: int = 30) -> Dict[str, Any]:
        ...
        for purchase in period_purchases:
            project = next((p for p in self.available_projects if p.project_id == purchase.project_id), None)
            if project:
        ...
        return {
            ...
            "total_cost_usd": total_cost_usd,
            "average_cost_per_kg": total_cost_usd / total_offset_kg if total_offset_kg > 0 else 0,
    ...
    def verify_offset_certificate(self, certificate_id: str) -> Dict[str, Any]:
        ...
        purchase = next((p for p in self.purchase_history if p.offset_certificate_id == certificate_id), None)
        ...
        return {
            ...
            "verification_date": purchase.verification_date.isoformat() if purchase.verification_date else None
        }
    ...
    def get_carbon_neutrality_status(self) -> Dict[str, Any]:
        ...
        neutrality_ratio = total_offset_kg / total_ai_carbon_kg if total_ai_carbon_kg > 0 else 0
        ...
        return {
            ...
            "total_offset_cost_usd": sum(p.total_cost_usd for p in self.purchase_history)
        }


# Utility functions

def create_carbon_offset_marketplace(config: Optional[CarbonOffsetConfig] = None) -> CarbonOffsetMarketplace:
    """Create and configure carbon offset marketplace."""
    return CarbonOffsetMarketplace(config)


def auto_offset_carbon_footprint(carbon_footprint: CarbonFootprint,
                               ai_experiment_id: Optional[str] = None,
                               config: Optional[CarbonOffsetConfig] = None) -> Optional[CarbonOffsetPurchase]:
...
def get_carbon_neutrality_report(config: Optional[CarbonOffsetConfig] = None) -> Dict[str, Any]:
