...
class CarbonAwareScheduler:
    ...
    def should_train_now(self) -> Tuple[bool, str, float]:
        ...
        if current_intensity <= self.config.low_carbon_threshold:
            return True, "Low carbon intensity - optimal time to train", current_intensity
        elif current_intensity <= self.config.medium_carbon_threshold:
            return True, "Medium carbon intensity - acceptable to train", current_intensity
        else:
            ...
            for time_point, intensity in forecast:
                if intensity <= self.config.low_carbon_threshold:
                    ...
                    if wait_hours <= self.config.max_wait_hours:
                        return False, f"High carbon intensity - wait {wait_hours:.1f}h for cleaner energy", current_intensity
            
            # If no clean period found within max wait time, train anyway
            return True, "High carbon intensity - but max wait time exceeded", current_intensity
    ...
    def wait_for_clean_energy(self, max_wait_hours: Optional[float] = None) -> float:
        ...
        while True:
            ...
            if elapsed_hours >= max_wait:
                logger.warning(f"Max wait time exceeded, starting training anyway ({intensity:.0f} gCO2/kWh)")
                return intensity
...
class CarbonBudgetEnforcer:
    ...
    def check_budget_status(self) -> Tuple[bool, float, str]:
        ...
        if usage_percentage >= self.config.emergency_stop_threshold * 100:
            return False, remaining_budget, f"EMERGENCY STOP: {usage_percentage:.1f}% of daily budget used"
        elif usage_percentage >= 80:
            return True, remaining_budget, f"WARNING: {usage_percentage:.1f}% of daily budget used"
        else:
            return True, remaining_budget, f"OK: {usage_percentage:.1f}% of daily budget used"
    
    def add_carbon_usage(self, carbon_kg: float):
        """Add carbon usage to daily total."""
        self.daily_usage += carbon_kg
        logger.debug(f"Added {carbon_kg:.6f} kg CO2e to daily usage (total: {self.daily_usage:.6f} kg)")
    
    def estimate_training_carbon(self, model: nn.Module, dataset_size: int, 
                               epochs: int, carbon_intensity: float) -> float:
        ...
        energy_per_param_per_sample = 1e-12  # kWh per parameter per sample (rough estimate)
        estimated_energy_kwh = model_params * dataset_size * epochs * energy_per_param_per_sample
...
class CarbonAwareOptimizer:
    ...
    def optimize_training(self, model: nn.Module, train_func: Callable, 
        ...
        if self.config.enable_budget_enforcement:
            can_continue, remaining_budget, status = self.budget_enforcer.check_budget_status()
        ...
        if self.config.enable_dynamic_scaling:
            ...
            if scale_factor < 1.0:
                ...
                logger.info(f"Model scaled to {scale_factor:.1%} due to high carbon intensity")
        ...
        if self.config.enable_adaptive_precision:
            ...
            optimized_model = self.precision_manager.apply_precision(optimized_model, precision)
            optimization_report["strategies_applied"].append(f"adaptive_precision_{precision}")
        ...
        if baseline_intensity and optimized_intensity:
            intensity_reduction = (baseline_intensity - optimized_intensity) / baseline_intensity
        ...
        logger.info(f"Carbon-optimized training completed: "
...
def carbon_aware_training(model: nn.Module, train_func: Callable, 
...
def get_carbon_status() -> Dict[str, Any]:
    ...
    can_continue, remaining_budget, budget_status = optimizer.budget_enforcer.check_budget_status()
