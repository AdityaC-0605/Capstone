...
class EnsembleConfig:
    ...
    weight_optimization_method: str = "validation_performance"  # 'validation_performance', 'bayesian', 'grid_search'
    
    # Stacking configuration
    meta_learner: str = "logistic_regression"  # 'logistic_regression', 'random_forest', 'neural_network'
    ...
    contribution_method: str = "shapley"  # 'shapley', 'weight_based', 'prediction_variance'
...
class EnsembleModel:
    ...
    def add_model(self, model_id: str, model: Any, model_type: str, 
                  weight: float = 1.0, performance_metrics: Optional[Dict[str, float]] = None) -> bool:
        """
        ...
        if len(self.models) >= self.config.max_models:
            logger.warning(f"Maximum number of models ({self.config.max_models}) reached")
            return False
        ...
        if auc_score < self.config.min_model_performance:
            logger.warning(f"Model {model_id} performance ({auc_score:.3f}) below threshold "
    ...
    def get_model_predictions(self, X: Union[pd.DataFrame, np.ndarray]) -> Dict[str, np.ndarray]:
            ...
            try:
                ...
                if hasattr(model, 'predict_proba'):
                    ...
                    if isinstance(model, (DNNModel, LSTMModel, GNNModel, TCNModel)):
                        ...
                        predictions[model_id] = probs[:, 1]  # Positive class probability
                    else:
                        # Sklearn-like models
    ...
    def fit(self, X: Union[pd.DataFrame, np.ndarray], y: Union[pd.Series, np.ndarray]) -> None:
        ...
        predictions_array = np.column_stack([model_predictions[mid] for mid in model_ids])
        ...
        logger.info(f"Ensemble fitted with {len(model_predictions)} models using {self.config.ensemble_method}")
    ...
    def get_model_contributions(self, X: Union[pd.DataFrame, np.ndarray]) -> Dict[str, float]:
        """
        ...
        if self.config.contribution_method == "weight_based":
            ...
            if isinstance(self.ensemble_method, WeightedAverageEnsemble):
                model_ids = [mid for mid in self.models.keys() if self.models[mid].is_active]
                for i, model_id in enumerate(model_ids):
                    contributions[model_id] = float(self.ensemble_method.weights[i])
            else:
                # Equal contributions for non-weighted methods
                active_models = [mid for mid in self.models.keys() if self.models[mid].is_active]
            ...
            for model_id, variance in variances.items():
                contributions[model_id] = variance / total_variance if total_variance > 0 else 0.0
    ...
    def calculate_diversity_metrics(self, X: Union[pd.DataFrame, np.ndarray]) -> Dict[str, float]:
...
class EnsembleTrainer:
    ...
    def train_and_evaluate(self, models: Dict[str, Any], X: pd.DataFrame, y: pd.Series,
                          test_size: float = 0.2) -> EnsembleResult:
        """
        ...
        try:
            ...
            if self.config.ensemble_method in ['stacking', 'blending']:
                if self.config.ensemble_method == 'blending':
                    ...
                    X_train, X_blend, y_train, y_blend = train_test_split(
                        X_train, y_train, test_size=self.config.holdout_ratio, 
            ...
            logger.info(f"Data split - Train: {len(X_train)}, Blend: {len(X_blend)}, Test: {len(X_test)}")
            ...
            for model_id, model in models.items():
                try:
                    ...
                    perf_metrics = self._evaluate_individual_model(model, X_test, y_test, model_id)
                    ...
                    if success:
                        logger.info(f"Added {model_id} to ensemble (AUC: {perf_metrics.get('roc_auc', 0.0):.4f})")
                    else:
    ...
    def _evaluate_individual_model(self, model: Any, X_test: pd.DataFrame, 
                                  y_test: pd.Series, model_id: str) -> Dict[str, float]:
        ...
        try:
            ...
            logger.error(f"Error evaluating model {model_id}: {e}")
            return {'roc_auc': 0.0, 'f1_score': 0.0, 'accuracy': 0.0, 'precision': 0.0, 'recall': 0.0}
    
    def _evaluate_ensemble(self, ensemble: EnsembleModel, X_test: pd.DataFrame, 
    ...
    def _get_model_weights(self, ensemble: EnsembleModel) -> Dict[str, float]:
        ...
        if isinstance(ensemble.ensemble_method, WeightedAverageEnsemble):
            active_models = [mid for mid in ensemble.models.keys() if ensemble.models[mid].is_active]
            if hasattr(ensemble.ensemble_method, 'weights') and ensemble.ensemble_method.weights is not None:
...
# Utility functions
def create_ensemble_from_results(results: Dict[str, Any], config: Optional[EnsembleConfig] = None) -> EnsembleModel:
    """
...
def train_ensemble_from_models(models: Dict[str, Any], X: pd.DataFrame, y: pd.Series,
