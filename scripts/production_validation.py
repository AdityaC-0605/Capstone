...
class ProductionValidator:
    ...
    def validate_api_health(self) -> bool:
        ...
        try:
            ...
            if health_response.status_code != 200:
                self.log_result("API Health", False, f"Health endpoint returned {health_response.status_code}")
                return False
            
            # Readiness check
            ready_response = self.session.get(f"{self.base_url}/ready")
            if ready_response.status_code != 200:
                self.log_result("API Readiness", False, f"Ready endpoint returned {ready_response.status_code}")
                return False
    ...
    def validate_prediction_accuracy(self) -> bool:
        ...
        try:
            ...
            if response.status_code != 200:
                self.log_result("Prediction Accuracy", False, f"Prediction endpoint returned {response.status_code}")
                return False
            
            result = response.json()
            
            # Validate response structure
            required_fields = ['risk_score', 'risk_category', 'confidence', 'explanation']
            missing_fields = [field for field in required_fields if field not in result]
            if missing_fields:
                self.log_result("Prediction Accuracy", False, f"Missing fields: {missing_fields}")
                return False
            
            # Validate risk score is reasonable for low-risk case
            risk_score = result['risk_score']
            if not (0 <= risk_score <= 1):
                self.log_result("Prediction Accuracy", False, f"Risk score {risk_score} out of valid range [0,1]")
                return False
            
            # For this low-risk case, we expect low risk score
            if risk_score > 0.5:
                self.log_result("Prediction Accuracy", False, f"Unexpected high risk score {risk_score} for low-risk case")
                return False
            
            self.log_result("Prediction Accuracy", True, f"Prediction validation passed (risk_score: {risk_score})")
            return True
    ...
    def validate_batch_processing(self) -> bool:
        ...
        try:
            ...
            if response.status_code != 200:
                self.log_result("Batch Processing", False, f"Batch endpoint returned {response.status_code}")
                return False
            
            result = response.json()
            
            if 'predictions' not in result:
                self.log_result("Batch Processing", False, "Missing 'predictions' in response")
                return False
            
            predictions = result['predictions']
            if len(predictions) != 2:
                self.log_result("Batch Processing", False, f"Expected 2 predictions, got {len(predictions)}")
                return False
            
            # Validate each prediction
            for i, prediction in enumerate(predictions):
                required_fields = ['application_id', 'risk_score', 'risk_category', 'confidence']
                missing_fields = [field for field in required_fields if field not in prediction]
                if missing_fields:
                    self.log_result("Batch Processing", False, f"Prediction {i} missing fields: {missing_fields}")
                    return False
            
            self.log_result("Batch Processing", True, "Batch processing validation passed")
            return True
    ...
    def validate_response_times(self) -> bool:
        ...
        try:
            ...
            if response.status_code != 200:
                self.log_result("Response Times", False, f"Prediction failed with status {response.status_code}")
                return False
            
            # SLA requirement: < 100ms per prediction
            if response_time_ms > 100:
                self.log_result("Response Times", False, f"Response time {response_time_ms:.2f}ms exceeds 100ms SLA")
                return False
            
            self.log_result("Response Times", True, f"Response time {response_time_ms:.2f}ms meets SLA")
            return True
    ...
    def validate_model_explainability(self) -> bool:
        ...
        try:
            ...
            if response.status_code != 200:
                self.log_result("Model Explainability", False, f"Prediction failed with status {response.status_code}")
                return False
            ...
            if 'explanation' not in result:
                self.log_result("Model Explainability", False, "Missing explanation in response")
                return False
            ...
            if not any(field in explanation for field in required_explanation_fields):
                self.log_result("Model Explainability", False, "Missing feature importance or SHAP values")
                return False
            
            self.log_result("Model Explainability", True, "Model explanations are provided")
            return True
    ...
    def validate_sustainability_metrics(self) -> bool:
        ...
        try:
            ...
            if response.status_code != 200:
                self.log_result("Sustainability Metrics", False, f"Metrics endpoint returned {response.status_code}")
                return False
            ...
            found_metrics = [metric for metric in sustainability_metrics if metric in metrics_content]
            
            if not found_metrics:
                self.log_result("Sustainability Metrics", False, "No sustainability metrics found")
                return False
            
            self.log_result("Sustainability Metrics", True, f"Found sustainability metrics: {found_metrics}")
            return True
    ...
    def validate_security_headers(self) -> bool:
        ...
        try:
            ...
            if response.status_code != 200:
                self.log_result("Security Headers", False, f"Health endpoint returned {response.status_code}")
                return False
            ...
            self.log_result("Security Headers", True, "Security headers are properly configured")
            return True
