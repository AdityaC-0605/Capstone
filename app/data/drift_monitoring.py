    ...
    def detect_numerical_drift(
        ...
        try:
            ...
            ks_statistic, p_value = ks_2samp(
                reference_data.dropna(), current_data.dropna()
            )
        ...
        try:
            ...
            if len(self.performance_history) > 1:
                ...
                if self.reference_performance:
                    ref_accuracy = self.reference_performance.get(
                        "accuracy", avg_recent_accuracy
                    )
...
class DataQualityMonitor:
    ...
    def assess_data_quality(
        ...
        try:
            ...
            # Schema violations
            schema_violations = (
                self._check_schema_violations(data, schema) if schema else 0
            )
            ...
            validity_score = (
                max(0, 1 - schema_violations / len(data.columns))
...
class DriftMonitor(DataProcessor):
    ...
    def set_reference_data(
        self, reference_data: pd.DataFrame, target_column: Optional[str] = None
    ):
        ...
        if target_column and target_column in reference_data.columns:
            ...
            if len(X_encoded.columns) > 0:
                try:
                    ...
                    reference_metrics = (
                        self.concept_detector._calculate_performance_metrics(
    ...
    def process(
        ...
        prediction_probabilities: Optional[pd.Series] = None,
        ...
        try:
            ...
            data_drift_detected, data_drift_scores = self._detect_data_drift(
                current_data
            )
            ...
            if target_column and predictions is not None:
                if target_column in current_data.columns:
                    concept_drift_detected, concept_metrics, concept_info = (
                        self.concept_detector.detect_concept_drift(
            ...
            if (
                quality_metrics_obj.missing_data_percentage
                > self.config.missing_data_threshold * 100
            ):
                alert = self.alert_manager.create_alert(
                ...
                if alert:
                    alerts.append(alert)
                recommendations.append(
                    "Investigate data collection pipeline for missing data issues"
                )

            if (
                quality_metrics_obj.duplicate_percentage
                > self.config.duplicate_threshold * 100
            ):
                alert = self.alert_manager.create_alert(
                    DriftType.DATA_DRIFT,
                    AlertSeverity.LOW,
    ...
    def _detect_data_drift(
        ...
        for column in common_columns:
            ...
            if pd.api.types.is_numeric_dtype(ref_series):
                ...
                column_drift, drift_score, test_info = (
                    self.statistical_detector.detect_numerical_drift(
                ...
                column_drift, drift_score, test_info = (
                    self.statistical_detector.detect_categorical_drift(
                        ref_series, curr_series
                    )
                )
            ...
            if column_drift:
                ...
                alert = self.alert_manager.create_alert(
                    ...
                    (
                        ...
                        if pd.api.types.is_numeric_dtype(ref_series)
                        else self.config.chi2_threshold
                    ),
            ...
            # Also calculate PSI
            psi_drift, psi_score = self.statistical_detector.calculate_psi(
                ref_series, curr_series
            )
        ...
        try:
            ...
            for col in self.reference_data.columns:
                if (
                    ...
                    and pd.api.types.is_numeric_dtype(self.reference_data[col])
