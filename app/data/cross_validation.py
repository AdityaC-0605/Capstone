...
from sklearn.model_selection import (
    StratifiedKFold, KFold, TimeSeriesSplit, ParameterGrid, ParameterSampler,
...
class CVConfig:
    ...
    scoring_metrics: List[str] = field(default_factory=lambda: ['accuracy', 'precision', 'recall', 'f1', 'roc_auc'])
...
class StratifiedKFoldValidator(BaseCrossValidator):
    ...
    def split(self, X: pd.DataFrame, y: pd.Series) -> Iterator[CVFold]:
        ...
        for fold_id, (train_idx, test_idx) in enumerate(self.cv.split(X, y)):
            ...
            stratification_info = {
                'train_class_distribution': train_y.value_counts(normalize=True).to_dict(),
                'test_class_distribution': test_y.value_counts(normalize=True).to_dict(),
    ...
    def validate_data(self, X: pd.DataFrame, y: pd.Series) -> bool:
        ...
        if min_class_count < self.config.n_splits:
            logger.error(f"Insufficient samples for stratification: min class has {min_class_count} samples, need at least {self.config.n_splits}")
            return False
...
class TimeSeriesValidator(BaseCrossValidator):
    ...
    def split(self, X: pd.DataFrame, y: pd.Series) -> Iterator[CVFold]:
        ...
        for fold_id, (train_idx, test_idx) in enumerate(self.cv.split(X)):
            ...
            temporal_info = {
                ...
                'gap_size': test_idx[0] - train_idx[-1] - 1 if len(train_idx) > 0 else 0
            }
    ...
    def validate_data(self, X: pd.DataFrame, y: pd.Series) -> bool:
        ...
        if len(X) < min_samples_needed:
            logger.error(f"Insufficient samples for time series CV: have {len(X)}, need at least {min_samples_needed}")
            return False
...
class NestedCrossValidator:
    ...
    def validate_estimator(self, estimator: BaseEstimator, param_grid: Dict[str, List],
                          X: pd.DataFrame, y: pd.Series) -> NestedCVResult:
        ...
        logger.info(f"Starting nested CV with {self.config.outer_cv_splits} outer and {self.config.inner_cv_splits} inner folds")
        
        for outer_fold, (train_outer_idx, test_outer_idx) in enumerate(self.outer_cv.split(X, y)):
            logger.info(f"Processing outer fold {outer_fold + 1}/{self.config.outer_cv_splits}")
            ...
            for params in ParameterGrid(param_grid):
                ...
                inner_fold_scores = {metric: [] for metric in self.config.scoring_metrics}
                
                for train_inner_idx, test_inner_idx in self.inner_cv.split(X_train_outer, y_train_outer):
                    ...
                    # Calculate scores
                    scores = self._calculate_scores(y_test_inner, y_pred_inner, estimator_clone, X_test_inner)
                    for metric, score in scores.items():
                        inner_fold_scores[metric].append(score)
                
                # Average inner CV scores
                avg_inner_scores = {metric: np.mean(scores) for metric, scores in inner_fold_scores.items()}
            ...
            outer_fold_scores = self._calculate_scores(y_test_outer, y_pred_outer, best_estimator, X_test_outer)
        ...
        generalization_score = {metric: np.mean(scores) for metric, scores in outer_scores.items()}
        hyperparameter_stability = self._calculate_hyperparameter_stability(best_params_per_fold)
    ...
    def _calculate_scores(self, y_true: pd.Series, y_pred: np.ndarray, 
                         estimator: BaseEstimator, X_test: pd.DataFrame) -> Dict[str, float]:
        ...
        try:
            ...
            if 'roc_auc' in self.config.scoring_metrics and hasattr(estimator, 'predict_proba'):
                try:
                    y_proba = estimator.predict_proba(X_test)
                    if y_proba.shape[1] == 2:  # Binary classification
                        scores['roc_auc'] = roc_auc_score(y_true, y_proba[:, 1])
                    else:  # Multiclass
                        scores['roc_auc'] = roc_auc_score(y_true, y_proba, multi_class='ovr', average='weighted')
                except:
                    scores['roc_auc'] = 0.0
    ...
    def _calculate_hyperparameter_stability(self, best_params_per_fold: List[Dict[str, Any]]) -> Dict[str, float]:
        ...
        for param_name in all_param_names:
            ...
            if all(isinstance(v, (int, float)) for v in param_values if v is not None):
                ...
                if param_values:
                    stability[param_name] = 1.0 - (np.std(param_values) / (np.mean(param_values) + 1e-8))
            else:
                # Categorical parameter - use mode frequency
                param_counts = pd.Series(param_values).value_counts()
                stability[param_name] = param_counts.iloc[0] / len(param_values) if len(param_counts) > 0 else 0.0
...
class CrossValidationManager(DataProcessor):
    ...
    def process(self, estimator: BaseEstimator, X: pd.DataFrame, y: pd.Series,
               param_grid: Optional[Dict[str, List]] = None) -> Union[CVResult, NestedCVResult]:
        """Process cross-validation."""
        start_time = datetime.now()
        
        try:
            logger.info(f"Starting cross-validation with strategy: {self.config.strategy.value}")
            
            # Nested CV if parameter grid is provided
            if param_grid and self.config.strategy == CVStrategy.NESTED_CV:
                return self.nested_validator.validate_estimator(estimator, param_grid, X, y)
            ...
            for fold in validator.split(X, y):
                logger.debug(f"Processing fold {fold.fold_id + 1}/{self.config.n_splits}")
                ...
                # Calculate scores
                fold_scores = self.nested_validator._calculate_scores(y_test, y_pred, estimator_clone, X_test)
            ...
            mean_scores = {metric: np.mean(score_list) for metric, score_list in scores.items()}
            std_scores = {metric: np.std(score_list) for metric, score_list in scores.items()}
    ...
    def validate(self, data: pd.DataFrame) -> bool:
        ...
        try:
            ...
            if len(data) < self.config.n_splits:
                logger.error(f"Insufficient data: {len(data)} samples for {self.config.n_splits} folds")
                return False
    ...
    def _perform_statistical_tests(self, scores: Dict[str, List[float]]) -> Dict[str, Any]:
...
def validate_model_cv(estimator: BaseEstimator, X: pd.DataFrame, y: pd.Series,
                     strategy: CVStrategy = CVStrategy.STRATIFIED_KFOLD,
                     n_splits: int = 5, config: Optional[CVConfig] = None) -> CVResult:
...
def nested_cv_hyperparameter_tuning(estimator: BaseEstimator, param_grid: Dict[str, List],
                                   X: pd.DataFrame, y: pd.Series,
                                   outer_cv: int = 5, inner_cv: int = 3,
                                   config: Optional[CVConfig] = None) -> NestedCVResult:
...
def get_time_series_cv_config(n_splits: int = 5, test_size: Optional[int] = None) -> CVConfig:
...
class CVResultAnalyzer:
    ...
    def analyze_cv_results(cv_result: CVResult) -> Dict[str, Any]:
        ...
        # Stability analysis
        primary_scores = cv_result.scores.get(cv_result.metadata.get('primary_metric', 'accuracy'), [])
        if primary_scores:
            analysis['stability'] = {
                'score_range': max(primary_scores) - min(primary_scores),
                'coefficient_of_variation': np.std(primary_scores) / np.mean(primary_scores),
        ...
        for metric, perf in analysis['performance'].items():
            if perf['cv'] > 0.2:  # High coefficient of variation
                recommendations.append(f"High variance in {metric} scores - consider more data or regularization")
        ...
        if primary_metric in analysis['performance']:
            ...
            if primary_perf < 0.7:
                recommendations.append("Low performance - consider feature engineering or different algorithms")
        
        # Check stability
        if not analysis['stability'].get('is_stable', True):
            recommendations.append("Unstable performance across folds - check for data leakage or overfitting")
    ...
    def analyze_nested_cv_results(nested_result: NestedCVResult) -> Dict[str, Any]:
        ...
        if nested_result.best_params_per_fold:
            ...
            for param in all_params:
                values = [params.get(param) for params in nested_result.best_params_per_fold]
                unique_values = len(set(v for v in values if v is not None))
                param_consistency[param] = {
                    'unique_values': unique_values,
                    'consistency_ratio': 1.0 - (unique_values - 1) / len(nested_result.best_params_per_fold),
        ...
        primary_score = list(nested_result.generalization_score.values())[0] if nested_result.generalization_score else 0
        if primary_score < 0.7:
            recommendations.append("Low generalization performance - consider simpler models or more data")
        
        # Check parameter stability
        unstable_params = [
            param for param, stability in nested_result.hyperparameter_stability.items()
            if stability < 0.5
        ]
        if unstable_params:
            recommendations.append(f"Unstable hyperparameters: {unstable_params} - consider fixing these parameters")
    ...
    @staticmethod
    def compare_cv_results(results: List[CVResult], labels: Optional[List[str]] = None) -> Dict[str, Any]:
            ...
            if len(valid_scores) >= 2:
                try:
                    ...
                    if all(len(scores) >= 3 for scores in valid_scores):
                        # Check normality
                        normal_tests = [stats.shapiro(scores)[1] > 0.05 for scores in valid_scores]
        ...
        if primary_metric in comparison['performance_comparison']:
            ...
            recommendations.append(f"Best performing model: {labels[best_idx]} ({primary_metric}: {means[best_idx]:.3f})")
        
        # Check for significant differences
        significant_metrics = [
            metric for metric, test in comparison['statistical_tests'].items()
            if test.get('significant', False)
        ]
        if significant_metrics:
            recommendations.append(f"Significant performance differences found in: {significant_metrics}")
...
def export_cv_results(cv_result: Union[CVResult, NestedCVResult], 
                     file_path: str, include_analysis: bool = True) -> bool:
    """Export cross-validation results to file."""
    try:
        export_data = {
            'timestamp': datetime.now().isoformat(),
            'cv_result': cv_result.__dict__ if hasattr(cv_result, '__dict__') else str(cv_result)
        }
        ...
        if include_analysis:
            if isinstance(cv_result, CVResult):
                ...
                export_data['analysis'] = CVResultAnalyzer.analyze_nested_cv_results(cv_result)
