...
class BehavioralFeatureExtractor:
    ...
    def extract_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if all(col in data.columns for col in ['num_open_credit_accounts', 'credit_score']):
            behavioral_features['credit_utilization_ratio'] = self._calculate_credit_utilization(data)
            behavioral_features['credit_account_density'] = data['num_open_credit_accounts'] / (data['age'] - 18 + 1)
        
        # Payment behavior patterns
        if 'num_delinquent_accounts' in data.columns:
            behavioral_features['delinquency_rate'] = data['num_delinquent_accounts'] / (data['num_open_credit_accounts'] + 1)
            behavioral_features['payment_reliability_score'] = self._calculate_payment_reliability(data)
        
        # Spending patterns (derived from loan purpose and amount)
        if all(col in data.columns for col in ['loan_purpose', 'loan_amount_inr', 'annual_income_inr']):
            behavioral_features['spending_category_risk'] = self._calculate_spending_risk(data)
            behavioral_features['loan_to_income_ratio'] = data['loan_amount_inr'] / data['annual_income_inr']
        
        # Risk appetite indicators
        if all(col in data.columns for col in ['loan_amount_inr', 'loan_term_months', 'annual_income_inr']):
            behavioral_features['risk_appetite_score'] = self._calculate_risk_appetite(data)
            behavioral_features['monthly_payment_burden'] = self._calculate_monthly_payment_burden(data)
        
        # Credit history depth
        if 'age' in data.columns:
            behavioral_features['potential_credit_history_years'] = np.maximum(data['age'] - 18, 0)
            behavioral_features['credit_maturity_score'] = self._calculate_credit_maturity(data)
        
        logger.info(f"Extracted {len(behavioral_features.columns) - len(data.columns)} behavioral features")
    ...
    def _calculate_credit_utilization(self, data: pd.DataFrame) -> pd.Series:
        ...
        base_utilization = (850 - data['credit_score']) / 550  # Inverse relationship with credit score
        account_factor = np.log1p(data['num_open_credit_accounts']) / 3  # More accounts = potentially higher utilization
    ...
    def _calculate_payment_reliability(self, data: pd.DataFrame) -> pd.Series:
        ...
        base_score = 1 - (data['num_delinquent_accounts'] / (data['num_open_credit_accounts'] + 1))
    ...
    def _calculate_monthly_payment_burden(self, data: pd.DataFrame) -> pd.Series:
        ...
        estimated_monthly_payment = (data['loan_amount_inr'] * 0.08/12) / (1 - (1 + 0.08/12)**(-data['loan_term_months']))
...
class FinancialFeatureExtractor:
    ...
    def extract_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'annual_income_inr' in data.columns:
            ...
            financial_features['income_stability_proxy'] = self._calculate_income_stability(data)
        
        # Loan amount features
        if 'loan_amount_inr' in data.columns:
            financial_features['loan_amount_percentile'] = self._calculate_percentile_rank(
        ...
        if 'debt_to_income_ratio' in data.columns:
            financial_features['dti_risk_category'] = self._categorize_dti_risk(data['debt_to_income_ratio'])
        ...
        if 'credit_score' in data.columns:
            ...
            financial_features['credit_score_category'] = self._categorize_credit_score(data['credit_score'])
            financial_features['credit_score_squared'] = (data['credit_score'] / 850) ** 2
        
        # Financial ratios and derived metrics
        if all(col in data.columns for col in ['loan_amount_inr', 'annual_income_inr', 'loan_term_months']):
            financial_features['loan_affordability_ratio'] = self._calculate_affordability_ratio(data)
            financial_features['financial_leverage'] = self._calculate_financial_leverage(data)
        
        # Employment type risk
        if 'employment_type' in data.columns:
            financial_features['employment_stability_score'] = self._calculate_employment_stability(data)
        
        logger.info(f"Extracted {len(financial_features.columns) - len(data.columns)} financial features")
    ...
    def _calculate_financial_leverage(self, data: pd.DataFrame) -> pd.Series:
        ...
        return data['loan_amount_inr'] / (data['annual_income_inr'] + data['loan_amount_inr'])
...
class TemporalFeatureExtractor:
    ...
    def extract_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'age' in data.columns:
            ...
            temporal_features['age_normalized'] = (data['age'] - 18) / (100 - 18)  # Normalize to 0-1
        
        # Loan term temporal features
        if 'loan_term_months' in data.columns:
            temporal_features['loan_term_years'] = data['loan_term_months'] / 12
            temporal_features['loan_term_category'] = self._categorize_loan_term(data['loan_term_months'])
            temporal_features['is_long_term_loan'] = (data['loan_term_months'] > 60).astype(int)
        
        # Life stage features
        if all(col in data.columns for col in ['age', 'marital_status']):
            temporal_features['life_stage'] = self._determine_life_stage(data)
            temporal_features['family_responsibility_score'] = self._calculate_family_responsibility(data)
        ...
        if all(col in data.columns for col in ['age', 'employment_type']):
            ...
            temporal_features['earning_potential_score'] = self._calculate_earning_potential(data)
        
        logger.info(f"Extracted {len(temporal_features.columns) - len(data.columns)} temporal features")
    ...
    def _categorize_age(self, age_series: pd.Series) -> pd.Series:
        ...
        return pd.cut(
            ...
            labels=['Young', 'Early Career', 'Mid Career', 'Senior Career', 'Pre-Retirement', 'Senior'],
            include_lowest=True
        )
    ...
    def _calculate_family_responsibility(self, data: pd.DataFrame) -> pd.Series:
        ...
        marital_factor = data['marital_status'].map({
            'Single': 0.0,
            'Married': 0.4,
            'Divorced': 0.2,
            'Widowed': 0.3
        ...
        age_factor = np.where(
            data['age'] < 35, (data['age'] - 18) / 17 * 0.3,
    ...
    def _calculate_earning_potential(self, data: pd.DataFrame) -> pd.Series:
        ...
        employment_potential = data['employment_type'].map({
            'Government': 0.7,
            'Salaried': 0.8,
            'Self-Employed': 0.6,
            'Unemployed': 0.1
        ...
        age_factor = np.where(
            data['age'] < 25, 0.3,
            np.where(data['age'] < 45, 0.3 + (data['age'] - 25) / 20 * 0.7,
...
class OutlierDetector:
    ...
    def detect_outliers(self, data: pd.DataFrame) -> Tuple[pd.DataFrame, np.ndarray]:
        ...
        logger.info(f"Detected {outlier_count} outliers using {self.config.outlier_method}")
...
class FeatureEngineeringPipeline(DataProcessor):
    ...
    def process(self, data: pd.DataFrame, target_column: str = 'default') -> FeatureEngineeringResult:
        ...
        try:
            ...
            # Balance classes
            X_balanced, y_balanced = self.class_balancer.balance_classes(X_clean, y_clean)
            
            # Calculate feature importance
            feature_importance = self._calculate_feature_importance(X_balanced, y_balanced)
            ...
            logger.info(f"Feature engineering completed. Final shape: {X_balanced.shape}")
            ...
            audit_logger.log_data_access(
                ...
                details={
                    ...
                    "outliers_removed": outlier_mask.sum() if len(outlier_mask) > 0 else 0,
    ...
    def validate(self, data: pd.DataFrame) -> bool:
        ...
        try:
            ...
            if missing_percentage > 0.5:
                logger.warning(f"High missing value percentage: {missing_percentage:.2%}")
                return False
            
            # Check feature variance
            numeric_features = data.select_dtypes(include=[np.number])
            zero_variance_features = numeric_features.columns[numeric_features.var() == 0]
            if len(zero_variance_features) > 0:
                logger.warning(f"Zero variance features detected: {list(zero_variance_features)}")
    ...
    def _encode_categorical_variables(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for column in categorical_columns:
            ...
            if unique_values <= 10:  # One-hot encode
                ...
                encoded_data = pd.concat([encoded_data.drop(columns=[column]), dummies], axis=1)
            else:  # Label encode
    ...
    def _calculate_feature_importance(self, X: pd.DataFrame, y: pd.Series) -> List[FeatureImportance]:
    ...
    def _classify_feature_type(self, feature_name: str) -> str:
        ...
        if any(keyword in feature_name_lower for keyword in ['behavior', 'payment', 'spending', 'credit_util']):
            return 'behavioral'
        elif any(keyword in feature_name_lower for keyword in ['income', 'loan', 'debt', 'financial', 'dti']):
            return 'financial'
        elif any(keyword in feature_name_lower for keyword in ['age', 'term', 'time', 'stage', 'years']):
            return 'temporal'
        elif any(keyword in feature_name_lower for keyword in ['account', 'relation', 'network']):
            return 'relational'
        else:
            return 'other'
...
def create_feature_engineering_pipeline(config: Optional[FeatureEngineeringConfig] = None) -> FeatureEngineeringPipeline:
    """Create a feature engineering pipeline instance."""
    return FeatureEngineeringPipeline(config)


def engineer_banking_features(data: pd.DataFrame, target_column: str = 'default', 
                             config: Optional[FeatureEngineeringConfig] = None) -> FeatureEngineeringResult:
