...
class LIMEExplanation:
    ...
    top_positive_factors: List[Tuple[str, float, str]]  # (feature, importance, description)
...
class LIMEExplainer:
    ...
    def __init__(self, model: nn.Module, config: Optional[LIMEConfig] = None):
        if not LIME_AVAILABLE:
            raise ImportError("LIME is required but not installed. Install with: pip install lime")
    ...
    def explain_instance(self, X: Union[pd.DataFrame, np.ndarray], 
                        instance_id: str = None) -> LIMEExplanation:
        """
        ...
        model_prediction = self.model_wrapper.predict_proba(X_array)[0, 1]  # Probability of positive class
        ...
        try:
            ...
            local_prediction = explanation.local_pred[1] if hasattr(explanation, 'local_pred') else model_prediction
            ...
            feature_names = self.config.feature_names or [f"feature_{i}" for i in range(len(instance))]
            local_explanation = [(name, 0.0) for name in feature_names[:self.config.num_features]]
            feature_importance = dict(local_explanation)
        ...
        simplified_explanation, top_positive, top_negative = self._create_customer_explanation(
        ...
        lime_explanation = LIMEExplanation(
            instance_id=instance_id or f"lime_instance_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
    ...
    def _create_customer_explanation(self, local_explanation: List[Tuple[str, float]], 
                                   prediction: float) -> Tuple[str, List[Tuple[str, float, str]], List[Tuple[str, float, str]]]:
        """Create simplified, customer-facing explanation."""
        
        # Sort by absolute importance
        sorted_explanation = sorted(local_explanation, key=lambda x: abs(x[1]), reverse=True)
        
        # Separate positive and negative factors
        positive_factors = [(name, value, self._get_factor_description(name, value, True)) 
                           for name, value in sorted_explanation if value > 0][:3]
        negative_factors = [(name, value, self._get_factor_description(name, value, False)) 
                           for name, value in sorted_explanation if value < 0][:3]
    ...
    def _get_factor_description(self, feature_name: str, importance: float, increases_risk: bool) -> str:
        """Generate customer-friendly description for a factor."""
        
        # Use custom names if provided
        if self.config.customer_friendly_names and feature_name in self.config.customer_friendly_names:
    ...
    def explain_batch(self, X: Union[pd.DataFrame, np.ndarray], 
                     instance_ids: Optional[List[str]] = None) -> List[LIMEExplanation]:
        """
    ...
    def _create_bar_plot(self, explanation: LIMEExplanation) -> str:
        ...
        try:
            ...
            ax.text(0.02, 0.98, f'Model Prediction: {explanation.model_prediction:.3f}\n'
    ...
    def _create_horizontal_bar_plot(self, explanation: LIMEExplanation) -> str:
        ...
        try:
            ...
            sorted_items = sorted(zip(features, importances), key=lambda x: abs(x[1]), reverse=True)
    ...
    def _create_waterfall_plot(self, explanation: LIMEExplanation) -> str:
        ...
        try:
            ...
            sorted_items = sorted(explanation.local_explanation, key=lambda x: abs(x[1]), reverse=True)
            features = [item[0] for item in sorted_items]
            importances = [item[1] for item in sorted_items]
            ...
            for i, (feature, imp) in enumerate(zip(features, importances)):
                ...
                ax.text(i, label_pos, f'{imp:.3f}', ha='center', va='center', 
                       fontweight='bold', color='white')
            
            # Add baseline and final prediction
            ax.axhline(y=explanation.intercept, color='blue', linestyle='--', 
                      label=f'Baseline: {explanation.intercept:.3f}')
            ax.axhline(y=explanation.local_prediction, color='orange', linestyle='-', 
            ...
            # Save plot
            if self.config.save_plots:
                plot_path = self._save_plot(fig, f"lime_waterfall_{explanation.instance_id}")
                plt.close(fig)
                return plot_path
    ...
    def _create_summary_plot(self, explanation: LIMEExplanation) -> str:
        ...
        try:
            ...
            ax3.pie(values, labels=labels, colors=colors_pie, autopct='%1.1f%%', startangle=90)
            ...
            # Add R² score
            ax4.text(0.5, 0.9, f'Local R²: {explanation.score:.3f}', 
            ...
            # Save plot
            if self.config.save_plots:
                plot_path = self._save_plot(fig, f"lime_summary_{explanation.instance_id}")
                plt.close(fig)
                return plot_path
    ...
    def save_explanations(self, explanations: List[LIMEExplanation], 
...
def create_lime_explainer(model: nn.Module, training_data: Union[pd.DataFrame, np.ndarray],
                         config: Optional[LIMEConfig] = None) -> LIMEExplainer:
    """
...
def explain_credit_decision(model: nn.Module, instance: Union[pd.DataFrame, np.ndarray],
                          ...
                          feature_names: List[str] = None) -> LIMEExplanation:
    """
...
def batch_explain_decisions(model: nn.Module, instances: Union[pd.DataFrame, np.ndarray],
                          training_data: Union[pd.DataFrame, np.ndarray],
                          feature_names: List[str] = None) -> List[LIMEExplanation]:
    """
