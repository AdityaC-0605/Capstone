...
class CounterfactualExplanation:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "counterfactual_predictions": [float(p) for p in self.counterfactual_predictions],
...
class CounterfactualGenerator(ABC):
    ...
    @abstractmethod
    def generate(self, instance: np.ndarray, feature_names: List[str]) -> List[np.ndarray]:
    ...
    def _calculate_sparsity(self, original: np.ndarray, counterfactual: np.ndarray, 
...
class GradientBasedGenerator(CounterfactualGenerator):
    ...
    def generate(self, instance: np.ndarray, feature_names: List[str]) -> List[np.ndarray]:
        ...
        target_class = 1 - original_class if self.config.target_class is None else self.config.target_class
        
        counterfactuals = []
        
        for attempt in range(self.config.num_counterfactuals * 2):  # Generate more, select best
...
class GeneticAlgorithmGenerator(CounterfactualGenerator):
    ...
    def generate(self, instance: np.ndarray, feature_names: List[str]) -> List[np.ndarray]:
        ...
        original_pred, original_class = self._get_prediction(torch.FloatTensor(instance))
        target_class = 1 - original_class if self.config.target_class is None else self.config.target_class
        ...
        def objective_function(x):
            ...
            # Distance penalty
            distance_penalty = self.config.lambda_distance * self._calculate_distance(instance, x)
            
            # Sparsity penalty
            sparsity_penalty = self.config.lambda_sparsity * self._calculate_sparsity(instance, x)
        ...
        for attempt in range(self.config.num_counterfactuals):
            try:
                ...
                if result.success:
                    ...
                    pred_prob, pred_class = self._get_prediction(torch.FloatTensor(candidate))
...
class DiceGenerator(CounterfactualGenerator):
    ...
    def generate(self, instance: np.ndarray, feature_names: List[str]) -> List[np.ndarray]:
        ...
        for i in range(self.config.num_counterfactuals):
            x = torch.FloatTensor(instance).to(self.device).clone().detach().requires_grad_(True)
            ...
            for iteration in range(self.config.max_iterations):
                ...
                # Distance penalty
                distance_loss = torch.norm(x - torch.FloatTensor(instance).to(self.device), p=2)
    ...
    def generate(self, instance: np.ndarray, feature_names: List[str]) -> List[np.ndarray]:
        ...
        for attempt in range(self.config.num_counterfactuals * 2):
            ...
            def closure():
                ...
                if target_class == 1:
                    prediction_loss = torch.max(torch.tensor(0.0).to(self.device), 
                                               0.5 - pred_prob) ** 2
                else:
                    prediction_loss = torch.max(torch.tensor(0.0).to(self.device), 
                                               pred_prob - 0.5) ** 2
                ...
                distance_loss = torch.norm(x - torch.FloatTensor(instance).to(self.device), p=1)
                
                total_loss = prediction_loss + self.config.lambda_distance * distance_loss
                total_loss.backward()
...
class CounterfactualExplainer:
    ...
    def __init__(self, model: nn.Module, config: Optional[CounterfactualConfig] = None):
        ...
        logger.info(f"Counterfactual explainer initialized with method: {self.config.method}")
    ...
    def explain(self, instance: np.ndarray, feature_names: List[str], 
                instance_id: str = None) -> CounterfactualExplanation:
        ...
        if not counterfactuals:
            ...
            return self._create_empty_explanation(instance, original_pred, original_class, instance_id)
    ...
    def _analyze_counterfactuals(self, original: np.ndarray, counterfactuals: List[np.ndarray],
                               feature_names: List[str], original_pred: float, 
                               original_class: int) -> Dict[str, List]:
    ...
    def _calculate_feasibility(self, counterfactual: np.ndarray, feature_names: List[str]) -> float:
    ...
    def _rank_counterfactuals(self, original: np.ndarray, counterfactuals: List[np.ndarray],
                            analysis_results: Dict[str, List]) -> List[int]:
        ...
        for i in range(len(counterfactuals)):
            ...
            max_distance = max(analysis_results["distances"]) if analysis_results["distances"] else 1.0
            normalized_distance = 1.0 - (distance / max_distance) if max_distance > 0 else 1.0
    ...
    def _create_empty_explanation(self, instance: np.ndarray, original_pred: float,
                                original_class: int, instance_id: str) -> CounterfactualExplanation:
        ...
        return CounterfactualExplanation(
            ...
            generation_method=self.config.method,
            generation_time=0.0,
            convergence_info={"num_generated": 0, "error": "No valid counterfactuals found"},
            ranked_indices=[],
    ...
    def batch_explain(self, instances: np.ndarray, feature_names: List[str],
                     instance_ids: Optional[List[str]] = None) -> List[CounterfactualExplanation]:
        ...
        for i, instance in enumerate(instances):
            ...
            try:
                ...
                logger.error(f"Failed to generate counterfactual for instance {instance_id}: {e}")
        ...
        logger.info(f"Generated counterfactual explanations for {len(explanations)} instances")
        return explanations
    
    def analyze_decision_boundary(self, instance: np.ndarray, feature_names: List[str],
                                feature_idx: int, num_points: int = 50) -> Dict[str, Any]:
        """Analyze decision boundary by varying a specific feature."""
        
        # Get original prediction
        original_pred, original_class = self.generator._get_prediction(torch.FloatTensor(instance))
        ...
        for val in feature_values:
            ...
            pred, pred_class = self.generator._get_prediction(torch.FloatTensor(modified_instance))
...
def generate_counterfactuals(model: nn.Module, instance: np.ndarray, 
                           feature_names: List[str], config: Optional[CounterfactualConfig] = None,
                           instance_id: str = None) -> CounterfactualExplanation:
    """
...
def analyze_what_if_scenarios(model: nn.Module, instance: np.ndarray,
                            feature_names: List[str], scenarios: Dict[str, float],
                            config: Optional[CounterfactualConfig] = None) -> Dict[str, Any]:
    """
    ...
    # Get original prediction
    original_pred, original_class = explainer.generator._get_prediction(torch.FloatTensor(instance))
    ...
    for scenario_name, modifications in scenarios.items():
        ...
        new_pred, new_class = explainer.generator._get_prediction(torch.FloatTensor(modified_instance))
        ...
        results["scenarios"][scenario_name] = {
            ...
            "distance": explainer.generator._calculate_distance(instance, modified_instance)
        }
    
    return results


def explore_decision_boundary(model: nn.Module, instance: np.ndarray,
                            feature_names: List[str], feature_name: str,
                            config: Optional[CounterfactualConfig] = None) -> Dict[str, Any]:
    """
