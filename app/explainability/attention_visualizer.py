...
class AttentionExplanation:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "temporal_attention": self.temporal_attention.tolist() if self.temporal_attention is not None else None,
            "peak_attention_timesteps": self.peak_attention_timesteps,
            "node_attention": self.node_attention.tolist() if self.node_attention is not None else None,
            "edge_attention": self.edge_attention.tolist() if self.edge_attention is not None else None,
    ...
    def get_model_prediction(self, X: torch.Tensor, **kwargs) -> float:
        ...
        with torch.no_grad():
            ...
            if len(output.shape) == 0 or (len(output.shape) == 1 and output.shape[0] == 1):
                ...
                prob = prob[1] if len(prob) > 1 else prob[0]  # Get positive class probability
...
class LSTMAttentionExtractor(AttentionExtractor):
    ...
    def extract_attention(self, X: torch.Tensor, lengths: Optional[torch.Tensor] = None) -> Dict[str, torch.Tensor]:
        ...
        with torch.no_grad():
            ...
            if hasattr(self.model, 'last_attention_weights') and self.model.last_attention_weights is not None:
                attention_weights = self.model.last_attention_weights
            elif hasattr(self.model, 'get_attention_weights'):
                attention_weights = self.model.get_attention_weights()
            elif hasattr(self.model, 'attention') and hasattr(self.model.attention, 'last_attention_weights'):
                ...
                if attention_weights is None:
                    ...
                    logger.warning("No attention weights found in LSTM model, using uniform weights")
            
            return {
                "temporal_attention": attention_weights,
                "sequence_lengths": lengths if lengths is not None else torch.full((X.shape[0],), X.shape[1])
            }
    
    def _extract_attention_with_hooks(self, X: torch.Tensor, lengths: Optional[torch.Tensor] = None) -> Optional[torch.Tensor]:
...
class GNNAttentionExtractor(AttentionExtractor):
        ...
        with torch.no_grad():
            ...
            if hasattr(self.model, 'conv_layers'):
                for i, layer in enumerate(self.model.conv_layers):
                    if hasattr(layer, 'attention_weights'):
                        attention_weights[f"layer_{i}_attention"] = layer.attention_weights
            ...
            if hasattr(self.model, 'pooling'):
                if hasattr(self.model.pooling, 'last_attention_weights'):
                    attention_weights["pooling_attention"] = self.model.pooling.last_attention_weights
            ...
            if not attention_weights:
                ...
                attention_weights["uniform_attention"] = torch.ones(num_nodes) / num_nodes
                logger.warning("No attention weights found in GNN model, using uniform weights")
    ...
    def _extract_pooling_attention(self, data) -> Optional[torch.Tensor]:
        ...
        try:
            if hasattr(self.model, 'pooling') and hasattr(self.model.pooling, 'attention'):
...
class AttentionVisualizer:
    ...
    def explain_lstm_attention(self, model: nn.Module, X: torch.Tensor, 
                             lengths: Optional[torch.Tensor] = None,
                             instance_id: str = None) -> AttentionExplanation:
        ...
        top_attended_features = [
            (f"timestep_{i}", float(attention_weights[i])) 
        ...
        explanation = AttentionExplanation(
            instance_id=instance_id or f"lstm_attention_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            model_type="LSTM",
    ...
    def explain_gnn_attention(self, model: nn.Module, data, 
                            instance_id: str = None) -> AttentionExplanation:
        ...
        top_indices = np.argsort(attention_weights)[-self.config.top_k_features:][::-1]
        top_attended_features = [
            (f"node_{i}", float(attention_weights[i])) 
            for i in top_indices
        ]
        ...
        explanation = AttentionExplanation(
            instance_id=instance_id or f"gnn_attention_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            model_type="GNN",
            model_prediction=prediction,
            attention_weights=attention_weights,
            attention_statistics=attention_stats,
    ...
    def _calculate_attention_statistics(self, attention_weights: np.ndarray) -> Dict[str, float]:
        ...
        stats = {
            ...
            "entropy": float(-np.sum(attention_weights * np.log(attention_weights + 1e-10))),
            "gini": float(self._calculate_gini_coefficient(attention_weights)),
            "concentration_ratio": float(np.sum(np.sort(attention_weights)[-3:]))  # Top 3 concentration
        }
    ...
    def create_temporal_heatmap(self, explanation: AttentionExplanation, 
        ...
        if explanation.model_type != "LSTM" or explanation.temporal_attention is None:
            logger.warning("Temporal heatmap only available for LSTM models with temporal attention")
            return None
        ...
        try:
            ...
            tick_positions = np.linspace(0, num_timesteps-1, min(10, num_timesteps), dtype=int)
            ...
            if self.config.show_values and num_timesteps <= 20:
                for i in range(num_timesteps):
                    text = ax.text(i, 0, f'{attention_data[0, i]:.3f}',
            ...
            if self.config.save_plots:
                plot_path = self._save_plot(fig, f"temporal_heatmap_{explanation.instance_id}")
                plt.close(fig)
                return plot_path
    ...
    def create_feature_attention_plot(self, explanation: AttentionExplanation) -> Optional[str]:
        ...
        try:
            ...
            top_features = explanation.top_attended_features[:self.config.top_k_features]
            ...
            for i, (bar, weight) in enumerate(zip(bars, weights)):
                ax1.text(weight + 0.001, i, f'{weight:.3f}', 
                        va='center', ha='left', fontsize=9)
            
            # Plot 2: Attention distribution histogram
            ax2.hist(explanation.attention_weights, bins=20, alpha=0.7, color='lightcoral')
...
Gini: {explanation.attention_statistics['gini']:.4f}"""
            ...
            # Save plot
            if self.config.save_plots:
                plot_path = self._save_plot(fig, f"feature_attention_{explanation.instance_id}")
                plt.close(fig)
                return plot_path
    ...
    def create_attention_flow_plot(self, explanation: AttentionExplanation) -> Optional[str]:
        """Create attention flow visualization showing how attention changes over time/nodes."""
        
        try:
            if explanation.model_type == "LSTM" and explanation.temporal_attention is not None:
                return self._create_temporal_flow_plot(explanation)
            elif explanation.model_type == "GNN" and explanation.node_attention is not None:
                return self._create_node_flow_plot(explanation)
            else:
                logger.warning("Attention flow plot requires temporal or node attention data")
                return None
    ...
    def _create_temporal_flow_plot(self, explanation: AttentionExplanation) -> Optional[str]:
        ...
        ax1.plot(timesteps, attention_weights, 'b-', linewidth=2, marker='o', markersize=4)
        ...
        if explanation.peak_attention_timesteps:
            peak_weights = [attention_weights[i] for i in explanation.peak_attention_timesteps]
            ax1.scatter(explanation.peak_attention_timesteps, peak_weights, 
                       color='red', s=100, zorder=5, label='Peak Attention')
            ax1.legend()
        ...
        # Save plot
        if self.config.save_plots:
            plot_path = self._save_plot(fig, f"attention_flow_{explanation.instance_id}")
            plt.close(fig)
            return plot_path
    ...
    def _create_node_flow_plot(self, explanation: AttentionExplanation) -> Optional[str]:
        ...
        ax2.plot(range(len(sorted_weights)), sorted_weights, 'ro-', linewidth=2, markersize=4)
    ...
    def generate_attention_report(self, explanation: AttentionExplanation) -> str:
        ...
        for i, (feature, weight) in enumerate(explanation.top_attended_features[:5], 1):
            ...
            report_lines.append(f"{i}. **{feature}**: {weight:.4f} ({percentage:.1f}% of total attention)")
        ...
        if attention_pattern == "highly concentrated":
            report_lines.append(f"focuses on a small number of key elements for its decision.")
        elif attention_pattern == "moderately concentrated":
            report_lines.append(f"balances focus between several important elements.")
        else:
            report_lines.append(f"considers many elements with relatively equal importance.")
    ...
    def create_comprehensive_attention_analysis(self, explanation: AttentionExplanation) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis["visualizations"]["temporal_heatmap"] = self.create_temporal_heatmap(explanation)
            analysis["visualizations"]["feature_attention"] = self.create_feature_attention_plot(explanation)
            analysis["visualizations"]["attention_flow"] = self.create_attention_flow_plot(explanation)
            ...
            logger.info(f"Comprehensive attention analysis completed for {explanation.instance_id}")
    ...
    def _generate_attention_insights(self, explanation: AttentionExplanation) -> Dict[str, Any]:
        ...
        insights["attention_concentration"] = {
            "level": "high" if entropy < 2.0 else "moderate" if entropy < 3.0 else "low",
            "entropy_score": entropy,
            "gini_coefficient": gini,
            "interpretation": "Focused decision-making" if entropy < 2.0 else "Balanced consideration" if entropy < 3.0 else "Distributed analysis"
        }
        ...
        if explanation.model_type == "LSTM" and explanation.temporal_attention is not None:
            ...
            dominant_phase = "early" if early_attention > max(mid_attention, late_attention) else \
                           "middle" if mid_attention > late_attention else "late"
            ...
            insights["temporal_patterns"] = {
                ...
                "attention_trend": "increasing" if late_attention > early_attention else "decreasing"
            }
        ...
        insights["decision_factors"] = {
            ...
            "decision_complexity": "simple" if total_top_attention > 0.7 else "moderate" if total_top_attention > 0.5 else "complex"
        }
        ...
        insights["model_behavior"] = {
            "confidence_level": "high" if max_attention > 0.5 else "moderate" if max_attention > 0.3 else "low",
            "attention_variance": explanation.attention_statistics['std'],
            "uniformity": "uniform" if explanation.attention_statistics['std'] < 0.1 else "varied"
        }
        
        return insights
    
    def _generate_compliance_data(self, explanation: AttentionExplanation) -> Dict[str, Any]:
        ...
        compliance_data["explainability_score"] = (concentration_score + entropy_score) / 2
        
        # Transparency metrics
        compliance_data["transparency_metrics"] = {
            "attention_coverage": len([w for w in explanation.attention_weights if w > 0.01]),
            "primary_factor_count": len([w for _, w in explanation.top_attended_features if w > 0.1]),
            "decision_interpretability": "high" if compliance_data["explainability_score"] > 0.7 else "moderate" if compliance_data["explainability_score"] > 0.5 else "low"
        }
        ...
        if compliance_data["explainability_score"] > 0.7:
            compliance_data["regulatory_notes"].append("High explainability - suitable for regulatory review")
        elif compliance_data["explainability_score"] > 0.5:
            compliance_data["regulatory_notes"].append("Moderate explainability - additional documentation may be required")
        else:
            compliance_data["regulatory_notes"].append("Low explainability - detailed review recommended")
        
        if explanation.model_type == "LSTM":
            compliance_data["regulatory_notes"].append("Temporal attention patterns available for sequence analysis")
        elif explanation.model_type == "GNN":
            compliance_data["regulatory_notes"].append("Graph attention patterns available for relationship analysis")
    ...
    def save_explanations(self, explanations: List[AttentionExplanation], 
...
def visualize_lstm_attention(model: nn.Module, X: torch.Tensor, 
                           lengths: Optional[torch.Tensor] = None,
                           config: Optional[AttentionConfig] = None) -> AttentionExplanation:
    """
...
def visualize_gnn_attention(model: nn.Module, data,
                          config: Optional[AttentionConfig] = None) -> AttentionExplanation:
    """
...
def create_attention_dashboard(explanations: List[AttentionExplanation],
                             config: Optional[AttentionConfig] = None) -> str:
    """
    ...
    fig = make_subplots(
        rows=2, cols=2,
        subplot_titles=('Attention Distribution', 'Model Predictions', 
                       'Attention Statistics', 'Temporal Patterns'),
    ...
    for i, exp in enumerate(explanations):
        ...
        fig.add_trace(
            go.Histogram(x=exp.attention_weights, name=f'{exp.instance_id}', 
        ...
        # Model predictions
        fig.add_trace(
            go.Scatter(x=[i], y=[exp.model_prediction], 
                      mode='markers', name=f'{exp.instance_id}',
                      marker=dict(size=10)),
            row=1, col=2
        )
    ...
    # Update layout
    fig.update_layout(
        title="Attention Analysis Dashboard",
        height=800,
        showlegend=True
    )
    ...
    try:
        ...
        dashboard_path = dashboard_dir / f"attention_dashboard_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
...
def batch_explain_attention(model: nn.Module, data_list: List, model_type: str = "auto",
                          config: Optional[AttentionConfig] = None) -> List[AttentionExplanation]:
    """
    ...
    logger.info(f"Processing {len(data_list)} instances for {model_type} attention analysis")
        ...
        try:
            ...
            if model_type.upper() == "LSTM":
                if isinstance(data, tuple):
                    ...
                    X, lengths = data, None
                explanation = visualizer.explain_lstm_attention(model, X, lengths, instance_id)
            else:  # GNN
...
def compare_attention_patterns(explanations: List[AttentionExplanation],
                             config: Optional[AttentionConfig] = None) -> Dict[str, Any]:
    """
    ...
    comparison["statistics"] = {
        ...
        "max_attention": {"mean": np.mean(all_max_attentions), "std": np.std(all_max_attentions)}
    }
    ...
    if comparison["patterns"]["concentrated_attention"] > 0.7:
        comparison["insights"].append("Most instances show concentrated attention patterns")
    elif comparison["patterns"]["distributed_attention"] > 0.7:
        comparison["insights"].append("Most instances show distributed attention patterns")
    else:
        comparison["insights"].append("Mixed attention patterns across instances")
    
    if comparison["patterns"]["consistent_patterns"]:
        comparison["insights"].append("Attention patterns are consistent across instances")
    else:
        comparison["insights"].append("Attention patterns vary significantly across instances")
