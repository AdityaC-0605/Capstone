...
class SHAPConfig:
    ...
    # Explainer settings
    explainer_type: str = (
        "auto"  # "auto", "tree", "deep", "linear", "kernel", "permutation"
    )
...
class SHAPExplainer:
    ...
    def __init__(self, model: nn.Module, config: Optional[SHAPConfig] = None):
        ...
        logger.info(
            f"SHAP explainer initialized with {self.config.explainer_type} explainer"
        )
    ...
    def _create_explainer(self):
        ...
        try:
            ...
            logger.warning(
                f"Failed to create {self.config.explainer_type} explainer: {e}"
            )
    ...
    def explain_instance(
        ...
        explanation = SHAPExplanation(
            ...
            or f"instance_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
    ...
    def explain_batch(
        ...
        if (
            ...
            and len(X_array) > self.config.batch_size
        ):
            # Process in batches
            for i in range(0, len(X_array), self.config.batch_size):
                ...
                logger.debug(
                    f"Processed batch {i//self.config.batch_size + 1}, instances {i}-{batch_end-1}"
                )
        else:
            # Process all at once
    ...
    def get_global_importance(
        self, X: Union[pd.DataFrame, np.ndarray], sample_size: int = None
    ) -> Dict[str, float]:
        """
        ...
        logger.info(
            f"Global importance calculated for {len(global_importance)} features"
        )
        return global_importance

    def create_visualization(
        self, explanation: SHAPExplanation, plot_type: str = None
    ) -> Optional[str]:
        """
    ...
    def _create_waterfall_plot(self, explanation: SHAPExplanation) -> str:
        ...
        try:
            ...
            n_features = min(
                self.config.max_display_features, len(explanation.shap_values)
            )
            ...
            for i, (pos, val, color) in enumerate(zip(positions, shap_values, colors)):
                ax.bar(
                    i,
                    abs(val),
                    bottom=pos if val > 0 else pos + val,
                    color=color,
                    alpha=0.7,
                    width=0.6,
                )
            ...
            ax.axhline(
                y=explanation.base_value,
                color="blue",
                linestyle="--",
                label=f"Base value: {explanation.base_value:.3f}",
            )
            ax.axhline(
                y=explanation.model_prediction,
                color="orange",
                linestyle="-",
        ...
        try:
            ...
            for i, (name, val, fval) in enumerate(negative_features):
                ...
                ax.barh(
                    0,
                    -width,
                    left=neg_cumsum - width,
                    height=0.5,
                    color="red",
                    alpha=0.7,
                    label=f"{name}={fval:.2f}" if i < 5 else "",
                )
            ...
            ax.axvline(
                x=explanation.model_prediction - explanation.base_value,
                color="orange",
                linestyle="-",
                linewidth=2,
                label=f"Prediction: {explanation.model_prediction:.3f}",
    ...
    def save_explanations(
        self, explanations: List[SHAPExplanation], filename: str = None
    ) -> str:
