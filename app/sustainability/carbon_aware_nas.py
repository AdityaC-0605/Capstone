...
class CarbonAwareNASConfig:
    ...
    # Primary objectives
    primary_objective: CarbonOptimizationObjective = (
        CarbonOptimizationObjective.BALANCED_SUSTAINABILITY
    )
    carbon_weight: float = (
        0.4  # Weight for carbon footprint in multi-objective optimization
    )
...
class CarbonAwareArchitecture:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "carbon_intensity_at_training": convert_numpy(
                self.carbon_intensity_at_training
            ),
...
class CarbonIntensityAPI:
    ...
    def get_current_carbon_intensity(self, region: str = "US") -> float:
        ...
        # Check cache first
        if (
            self.cached_intensity is not None
            and self.last_check
            and datetime.now() - self.last_check < self.cache_duration
        ):
    ...
    def is_optimal_for_training(
        self, region: str = "US", threshold: float = 300.0
    ) -> bool:
...
class CarbonAwareArchitectureEvaluator:
    ...
    def evaluate_architecture(
        ...
        try:
            ...
            if (
                self.config.enable_real_time_carbon
                and current_intensity > self.config.carbon_intensity_threshold
            ):
                logger.warning(
                    f"Skipping training due to high carbon intensity: {current_intensity:.1f} gCO2/kWh"
                )
            ...
            if architecture.estimated_carbon_kg > 0:
                architecture.carbon_efficiency_score = (
                    performance_metrics.get("roc_auc", 0.0)
    ...
    def _create_model_from_architecture(
        self, architecture: CarbonAwareArchitecture, input_dim: int
    ) -> nn.Module:
...
class CarbonAwareNAS:
    ...
    def search(self, X: pd.DataFrame, y: pd.Series) -> CarbonAwareArchitecture:
        ...
        logger.info(
            f"Carbon-aware NAS completed. Best architecture: {self.best_architecture.architecture_id}"
        )
    ...
    def _calculate_parameters(self, layers: List[Dict], input_dim: int) -> int:
        ...
        for layer in layers:
            if layer["type"] == "linear":
                ...
                total_params += (
                    current_dim * layer["hidden_size"] + layer["hidden_size"]
                )
    ...
    def _calculate_fitness(self, architecture: CarbonAwareArchitecture) -> float:
        ...
        if (
            ...
            == CarbonOptimizationObjective.MINIMIZE_CARBON_FOOTPRINT
        ):
            ...
            self.config.primary_objective
            == CarbonOptimizationObjective.MAXIMIZE_CARBON_EFFICIENCY
        ):
            ...
            energy_score = max(
                ...
                1
                - architecture.estimated_energy_mj
                / (self.config.max_energy_budget_kwh * 3.6),
            )
    ...
    def _crossover_and_mutation(self, input_dim: int):
        ...
        while len(new_population) < self.config.population_size:
            ...
            if np.random.random() < self.config.crossover_rate:
                ...
                child = self._generate_random_architecture(
                    input_dim, f"arch_{len(new_population)}"
                )
    ...
    def _crossover(
        self,
        parent1: CarbonAwareArchitecture,
        parent2: CarbonAwareArchitecture,
        input_dim: int,
    ) -> CarbonAwareArchitecture:
    ...
    def _mutate(
        ...
        for layer in architecture.layers:
            ...
            if np.random.random() < 0.1:  # 10% chance to toggle batch norm
                mutated_layer["use_batch_norm"] = not mutated_layer.get(
    ...
    def _update_best_architecture(self):
        ...
        if (
            ...
            logger.info(
                f"New best architecture: {self.best_architecture.architecture_id} "
    ...
    def _log_generation_progress(self):
        ...
        if evaluated_architectures:
            avg_performance = np.mean(
                [
                    a.performance_metrics.get("roc_auc", 0.0)
                    for a in evaluated_architectures
                ]
            )
            avg_carbon = np.mean(
                [a.estimated_carbon_kg for a in evaluated_architectures]
            )
            avg_energy = np.mean(
                [a.estimated_energy_mj for a in evaluated_architectures]
            )
    ...
    def _final_evaluation(self, X: pd.DataFrame, y: pd.Series):
        ...
        logger.info(
            f"Final evaluation complete. Best architecture: {self.best_architecture.architecture_id}"
        )
        ...
        summary = {
            ...
            "search_results": {
                ...
                "best_architecture_id": (
                    self.best_architecture.architecture_id
                    if self.best_architecture
                    else None
                ),
                "best_fitness_score": (
                ...
                "best_carbon_kg": (
                    self.best_architecture.estimated_carbon_kg
                    if self.best_architecture
                    else 0.0
                ),
                "best_energy_mj": (
                    self.best_architecture.estimated_energy_mj
                    if self.best_architecture
                    else 0.0
                ),
            },
    ...
    def get_carbon_efficiency_ranking(
        self,
    ...
    def get_energy_efficiency_ranking(
        self,
    ) -> List[Tuple[CarbonAwareArchitecture, float]]:
...
def compare_carbon_efficiency(
    architectures: List[CarbonAwareArchitecture],
) -> pd.DataFrame:
