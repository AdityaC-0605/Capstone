...
class AdaptivePruner:
    ...
    def prune_model(
        ...
        try:
            ...
            if pruning_ratio < self.config.min_pruning_ratio:
                logger.info(
                    f"Carbon intensity {carbon_intensity:.1f} gCO2/kWh too low for pruning"
                )
            ...
            energy_savings = (
                1 - compression_ratio
            ) * 0.7  # 70% of size reduction translates to energy savings
            carbon_savings = (
                energy_savings * 0.8
            )  # 80% of energy savings translates to carbon savings
            ...
            logger.info(
                f"Model pruned: {pruning_ratio:.1%} reduction, {energy_savings:.1%} energy savings"
            )
            return result
    ...
    def _calculate_pruning_ratio(self, carbon_intensity: float) -> float:
        ...
        if carbon_intensity <= self.config.low_carbon_threshold:
            ...
            ratio = (
                (carbon_intensity - self.config.low_carbon_threshold)
                / (
                    self.config.medium_carbon_threshold
                    - self.config.low_carbon_threshold
                )
                * 0.3
            )
            ...
            ratio = (
                0.3
                + (carbon_intensity - self.config.medium_carbon_threshold)
                / (
                    self.config.high_carbon_threshold
                    - self.config.medium_carbon_threshold
                )
                * 0.5
            )
    ...
    def _apply_structured_pruning(
        self, model: nn.Module, pruning_ratio: float
    ) -> nn.Module:
...
class AdaptiveQuantizer:
    ...
    def quantize_model(
        ...
        try:
            ...
            if quantization_level == "fp32":
                logger.info(
                    f"Carbon intensity {carbon_intensity:.1f} gCO2/kWh too low for quantization"
                )
                return ModelOptimizationResult(
            ...
            energy_savings = (
                1 - compression_ratio
            ) * 0.6  # 60% of size reduction translates to energy savings
            carbon_savings = (
                energy_savings * 0.7
            )  # 70% of energy savings translates to carbon savings
            ...
            result = ModelOptimizationResult(
                ...
                optimization_timestamp=datetime.now(),
                carbon_intensity_at_optimization=carbon_intensity,
                strategy_used=self._get_strategy_for_quantization_level(
                    quantization_level
                ),
                success=True,
            )
            ...
            logger.info(
                f"Model quantized to {quantization_level}: {compression_ratio:.1%} size reduction"
            )
            return result
    ...
    def _apply_quantization(
        self, model: nn.Module, quantization_level: str
    ) -> nn.Module:
    ...
    def _get_strategy_for_quantization_level(
        self, quantization_level: str
    ) -> OptimizationStrategy:
...
class KnowledgeDistillationManager:
    ...
    def create_student_model(
        self, teacher_model: nn.Module, carbon_intensity: float
    ) -> Tuple[nn.Module, ModelOptimizationResult]:
        ...
        try:
            ...
            energy_savings = (
                1 - compression_ratio
            ) * 0.8  # 80% of size reduction translates to energy savings
            carbon_savings = (
                energy_savings * 0.9
            )  # 90% of energy savings translates to carbon savings
            ...
            logger.info(
                f"Student model created: {compression_ratio:.1%} size reduction"
            )
            ...
            def __init__(self, original_model):
                ...
                for name, module in original_model.named_modules():
                    if isinstance(module, nn.Linear):
                        ...
                        new_out_features = max(
                            1, int(module.out_features * compression_ratio)
                        )
                        self.layers.append(
                            nn.Linear(module.in_features, new_out_features)
                        )
                    elif isinstance(module, (nn.Conv2d, nn.Conv1d)):
                        # Reduce output channels
                        new_out_channels = max(
                            1, int(module.out_channels * compression_ratio)
                        )
                        self.layers.append(
                            type(module)(
                                module.in_channels,
...
class SustainableModelLifecycleManager:
    ...
    def optimize_model_for_carbon(
        ...
        try:
            ...
            optimization_type = self._select_optimization_type(
                carbon_intensity, model_info
            )

            # Apply optimization
            if optimization_type == "pruning":
                result = self.pruner.prune_model(
    ...
    def _select_optimization_type(
        self, carbon_intensity: float, model_info: Dict[str, Any]
    ) -> str:
        ...
        recent_optimizations = [
            ...
            if (datetime.now() - opt.optimization_timestamp).total_seconds()
            < 3600  # Last hour
        ]
    ...
    def check_model_retirement(self, model_id: str) -> bool:
        ...
        if (
            model_info["carbon_efficiency_score"]
            < self.config.carbon_efficiency_threshold
        ):
            logger.info(
                f"Model {model_id} marked for retirement due to low carbon efficiency"
            )
            return True
    ...
    def get_system_lifecycle_summary(self) -> Dict[str, Any]:
        ...
        total_energy_savings = sum(
            opt.energy_savings_percent for opt in self.optimization_history
        )
        total_carbon_savings = sum(
            opt.carbon_savings_percent for opt in self.optimization_history
        )
        ...
        return {
            ...
            "optimization_success_rate": successful_optimizations
            / max(1, total_optimizations),
            "total_energy_savings_percent": total_energy_savings,
            "total_carbon_savings_percent": total_carbon_savings,
            "average_energy_savings_per_optimization": total_energy_savings
            / max(1, total_optimizations),
            "average_carbon_savings_per_optimization": total_carbon_savings
            / max(1, total_optimizations),
        }


# Utility functions
def create_sustainable_lifecycle_manager(
...
def demo_sustainable_model_lifecycle() -> Dict[str, Any]:
    ...
    for i, carbon_intensity in enumerate(carbon_scenarios):
        ...
        logger.info(
            f"Optimization {i+1}: {result.optimization_type}, "
