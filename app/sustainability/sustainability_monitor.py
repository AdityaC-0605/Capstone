...
class SustainabilityMonitor:
    ...
    def start_monitoring(self):
        ...
        if self.dashboard and self.config.enable_dashboard:
            dashboard_thread = threading.Thread(
                target=self._start_dashboard, daemon=True
            )
    ...
    def stop_experiment_tracking(self, experiment_id: str) -> Dict[str, Any]:
        ...
        if experiment_id not in self.active_experiments:
            raise ValueError(
                f"Experiment {experiment_id} not found in active experiments"
            )
        ...
        carbon_footprint = self.carbon_calculator.calculate_carbon_footprint(
            energy_report,
            region=(
                self.config.carbon_config.default_region
                if self.config.carbon_config
                else "US"
            ),
        )
        ...
        logger.info(
            f"Completed sustainability tracking for experiment: {experiment_id}"
        )
    ...
    def _check_alert_conditions(self):
        ...
        if (
            ...
            and self.config.carbon_config.enable_budget_monitoring
        ):
            try:
                # Check daily budget
    ...
    def _add_alert(self, alert: SustainabilityAlert):
        ...
        # Log alert
        logger.warning(
            f"Sustainability Alert [{alert.level.value.upper()}]: {alert.message}"
        )
...
def create_sustainability_monitor(
