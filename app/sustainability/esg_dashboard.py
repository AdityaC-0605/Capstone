...
class ESGDashboard:
    ...
    def __init__(
        ...
        if not DASH_AVAILABLE:
            raise ImportError(
                "Dash and Plotly are required for ESG Dashboard. "
    ...
    def _create_alerts_component(self, alerts: List[str]) -> html.Div:
        ...
        if not alerts:
            return html.Div(
                [
                    html.Div(
                        "✅ All ESG metrics are within acceptable ranges",
                        style={
                            "color": "green",
                            "fontWeight": "bold",
                            "textAlign": "center",
        ...
        for category in categories:
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    y=avg_performance["performance_ratio"]
    ...
    def _create_category_breakdown_chart(self, esg_score: ESGScore) -> go.Figure:
        ...
        fig.update_layout(title="ESG Score Breakdown", showlegend=True)

        return fig

    def _create_metrics_chart(
        self, metrics: List[ESGMetric], category: ESGCategory
    ) -> go.Figure:
        ...
        if not category_metrics:
            fig = go.Figure()
            fig.add_annotation(
                text=f"No {category.value} metrics available",
                xref="paper",
                yref="paper",
                x=0.5,
                y=0.5,
    ...
    def _create_target_progress_chart(self, metrics: List[ESGMetric]) -> go.Figure:
        ...
        if not metric_names:
            fig = go.Figure()
            fig.add_annotation(
                text="No target progress data available",
                xref="paper",
                yref="paper",
                x=0.5,
                y=0.5,
