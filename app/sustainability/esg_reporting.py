...
class ESGReportingConfig:
    ...
    preferred_training_hours: List[int] = field(default_factory=lambda: [2, 3, 4, 5, 6])  # 2-6 AM
...
class CarbonAwareScheduler:
    ...
    def get_optimal_training_time(self, duration_hours: float = 1.0,
                                region: str = "US") -> Tuple[datetime, float]:
        """
        ...
        current_intensity = self.carbon_calculator.energy_mix_db.get_carbon_intensity(region)
        ...
        for hour in self.config.preferred_training_hours:
            ...
            for day_offset in [0, 1]:
                ...
                if candidate_time > now:
                    ...
                    estimated_intensity = self._estimate_carbon_intensity(candidate_time, region)
        ...
        logger.info(f"Optimal training time: {best_time} (estimated {best_intensity:.1f} gCO2/kWh)")
    ...
    def _estimate_carbon_intensity(self, target_time: datetime, region: str) -> float:
        ...
        base_intensity = self.carbon_calculator.energy_mix_db.get_carbon_intensity(region)
    ...
    def should_start_training_now(self, region: str = "US") -> Tuple[bool, str]:
        ...
        if current_intensity <= self.config.carbon_intensity_threshold:
            ...
            optimal_time, optimal_intensity = self.get_optimal_training_time(region=region)
            return False, (f"High carbon intensity: {current_intensity:.1f} gCO2/kWh. "
...
class CarbonOffsetTracker:
    ...
    def create_offset_record(self, emissions_kg: float, 
                           offset_type: str = "renewable_energy",
                           provider: Optional[str] = None) -> CarbonOffset:
        ...
        logger.info(f"Carbon offset created: {offset_amount:.3f} kg CO2e for ${offset_cost:.2f}")
    ...
    def get_total_offsets(self, period_days: int = 30) -> Dict[str, float]:
        ...
        period_offsets = [
            offset for offset in self.offsets
...
class ESGReportGenerator:
    ...
    def __init__(self, config: Optional[ESGReportingConfig] = None,
        ...
        self.carbon_scheduler = CarbonAwareScheduler(self.config, self.esg_collector.carbon_calculator)
    ...
    def _load_default_templates(self) -> Dict[str, ReportTemplate]:
        ...
        templates["executive"] = ReportTemplate(
            ...
            metrics_included=["overall_esg_score", "carbon_emissions", "energy_consumption"],
        ...
        templates["regulatory"] = ReportTemplate(
            ...
            sections=["compliance_summary", "detailed_metrics", "audit_trail", "certifications"],
        ...
        templates["investor"] = ReportTemplate(
            ...
            format=ReportFormat.PDF,
            sections=["investment_summary", "esg_performance", "risk_analysis", "future_outlook"],
        ...
        templates["tcfd"] = ReportTemplate(
            ...
            metrics_included=["climate_metrics", "carbon_emissions", "energy_consumption"],
    ...
    def generate_report(self, template_id: str, period_days: int = 30,
    ...
    def _collect_report_data(self, period_days: int) -> Dict[str, Any]:
        ...
        carbon_footprints = self.esg_collector.carbon_calculator.carbon_history[-50:]  # Last 50
    ...
    def _generate_tcfd_report(self, data: Dict[str, Any], template: ReportTemplate) -> Dict[str, Any]:
        ...
        tcfd_report = {
            ...
            "metrics_and_targets": {
                "climate_metrics": {
                    ...
                    "scope2_emissions_kg": sum(fp["total_emissions_kg"] for fp in data["carbon_footprints"]),
                    "scope3_emissions_kg": sum(fp["embodied_emissions_kg"] for fp in data["carbon_footprints"]),
                    "energy_consumption_kwh": sum(fp["energy_kwh"] for fp in data["carbon_footprints"]),
                    "renewable_energy_percentage": data["esg_score"]["environmental_metrics"].get("renewable_energy_ratio", 0) * 100,
                    "carbon_intensity_gco2_kwh": data["esg_score"]["environmental_metrics"].get("carbon_intensity", 0)
                },
                "targets": {
    ...
    def _generate_sasb_report(self, data: Dict[str, Any], template: ReportTemplate) -> Dict[str, Any]:
        ...
        sasb_report = {
            ...
            "environmental_footprint": {
                ...
                "metrics": {
                    "total_energy_consumed_kwh": sum(fp["energy_kwh"] for fp in data["carbon_footprints"]),
                    "percentage_grid_electricity": 100,  # All from grid
                    "percentage_renewable": data["esg_score"]["environmental_metrics"].get("renewable_energy_ratio", 0) * 100
                }
            },
            ...
            "data_privacy_security": {
                ...
                "metrics": {
                    "data_privacy_score": data["esg_score"]["social_metrics"].get("data_privacy_score", 0) * 100,
                    "privacy_incidents": 0,
                    "users_affected": 0,
            ...
            "algorithmic_bias": {
                ...
                "metrics": {
                    "fairness_score": data["esg_score"]["social_metrics"].get("algorithmic_fairness", 0) * 100,
                    "bias_testing_frequency": "continuous",
                    "protected_attributes_monitored": ["race", "gender", "age"],
                    "bias_mitigation_techniques": ["reweighting", "adversarial_debiasing"]
                }
            },
            ...
            "managing_systemic_risks": {
                ...
                "metrics": {
                    ...
                    "risk_management_score": data["esg_score"]["governance_metrics"].get("risk_management", 0) * 100
                }
            }
        }
    ...
    def _generate_standard_report(self, data: Dict[str, Any], template: ReportTemplate) -> Dict[str, Any]:
        ...
        report = {
            ...
            "executive_summary": {
                ...
                "areas_for_improvement": data["recommendations"][:3]  # Top 3 recommendations
            },
            
            "environmental_performance": {
                "carbon_emissions": {
                    "total_kg": sum(fp["total_emissions_kg"] for fp in data["carbon_footprints"]),
                    "operational_kg": sum(fp["operational_emissions_kg"] for fp in data["carbon_footprints"]),
                    "embodied_kg": sum(fp["embodied_emissions_kg"] for fp in data["carbon_footprints"])
                },
                "energy_consumption": {
                    "total_kwh": sum(fp["energy_kwh"] for fp in data["carbon_footprints"]),
                    "efficiency_score": data["esg_score"]["environmental_metrics"].get("energy_efficiency", 0)
                },
                "carbon_offsets": data["carbon_offsets"]
            },
            
            "social_performance": {
                "algorithmic_fairness": data["esg_score"]["social_metrics"].get("algorithmic_fairness", 0),
                "data_privacy": data["esg_score"]["social_metrics"].get("data_privacy_score", 0),
                "transparency": data["esg_score"]["social_metrics"].get("transparency_score", 0),
                "accessibility": data["esg_score"]["social_metrics"].get("accessibility_score", 0)
            },
            
            "governance_performance": {
                "model_governance": data["esg_score"]["governance_metrics"].get("model_governance", 0),
                "data_governance": data["esg_score"]["governance_metrics"].get("data_governance", 0),
                "compliance": data["esg_score"]["governance_metrics"].get("compliance_score", 0),
                "risk_management": data["esg_score"]["governance_metrics"].get("risk_management", 0)
            },
    ...
    def _generate_html_report(self, report: Dict[str, Any], template: ReportTemplate) -> str:
        ...
        if JINJA2_AVAILABLE:
            ...
            html = html_template.replace("{{ report.report_type }}", str(report.get("report_type", "ESG Report")))
            html = html.replace("{{ report.generated_at }}", str(report.get("generated_at", datetime.now().isoformat())))
            return html
    ...
    def _generate_xml_report(self, report: Dict[str, Any]) -> str:
        ...
        ET.SubElement(metadata, "GeneratedAt").text = str(report.get("generated_at", ""))
        
        # Add ESG scores
        if "executive_summary" in report:
            scores = ET.SubElement(root, "ESGScores")
            summary = report["executive_summary"]
            ET.SubElement(scores, "Overall").text = str(summary.get("overall_esg_score", 0))
            ET.SubElement(scores, "Environmental").text = str(summary.get("environmental_score", 0))
            ET.SubElement(scores, "Social").text = str(summary.get("social_score", 0))
            ET.SubElement(scores, "Governance").text = str(summary.get("governance_score", 0))
    ...
    def start_scheduled_reporting(self):
        ...
        if not SCHEDULE_AVAILABLE:
            logger.warning("Schedule module not available - scheduled reporting disabled")
            return
        ...
        getattr(schedule.every(), self.config.weekly_report_day.lower()).at(self.config.daily_report_time).do(
            self._generate_scheduled_report, "investor", "weekly"
        )
    ...
    def _send_report_email(self, report: Dict[str, Any], template_id: str):
        ...
        if not all([self.config.smtp_server, self.config.smtp_username, self.config.smtp_password]):
            logger.warning("Email configuration incomplete - skipping email distribution")
            return
        ...
        stakeholder_emails = self.config.stakeholder_emails.get(template.stakeholder_type, [])
        if not stakeholder_emails:
            logger.warning(f"No email addresses configured for {template.stakeholder_type.value}")
            return
...
def create_esg_reporter(config: Optional[ESGReportingConfig] = None,
                       esg_collector: Optional[ESGMetricsCollector] = None,
                       sustainability_monitor: Optional[SustainabilityMonitor] = None) -> ESGReportGenerator:
    """Create ESG report generator."""
    return ESGReportGenerator(config, esg_collector, sustainability_monitor)


def generate_tcfd_report(esg_collector: Optional[ESGMetricsCollector] = None,
...
def generate_sasb_report(esg_collector: Optional[ESGMetricsCollector] = None,
                        period_days: int = 30) -> Dict[str, Any]:
