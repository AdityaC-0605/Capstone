...
class PerformanceValidator:
    ...
    def concurrent_requests_test(
        self, num_requests: int = 50, num_workers: int = 10
    ) -> Dict[str, Any]:
        ...
        for metric, threshold in sla_requirements.items():
            if metric.startswith("max_"):
                ...
                if results[actual_metric] > threshold:
                    violations.append(
                        f"{actual_metric}: {results[actual_metric]:.2f} > {threshold}"
                    )
    ...
    def run_performance_validation(
        self, num_requests: int = 100, num_workers: int = 10
    ) -> bool:
        """Run complete performance validation"""
        print(f"🚀 Running performance validation against {self.base_url}")
        print(
            f"Test parameters: {num_requests} requests, {num_workers} concurrent workers"
        )
        print("=" * 60)
