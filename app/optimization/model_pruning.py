...
class MagnitudePruner(BasePruner):
    ...
    def prune_model(self, model: nn.Module, config: PruningConfig) -> nn.Module:
        ...
        actual_sparsity = 1.0 - (pruned_params / total_params) if total_params > 0 else 0.0
    ...
    def _should_prune_layer(self, name: str, module: nn.Module, config: PruningConfig) -> bool:
...
class StructuredPruner(BasePruner):
    ...
    def _prune_neurons(self, model: nn.Module, config: PruningConfig) -> nn.Module:
        ...
        for name, module in model.named_modules():
            if isinstance(module, nn.Linear) and self._should_prune_layer(name, module, config):
...
class GradualPruner(BasePruner):
    ...
    def prune_model(self, model: nn.Module, config: PruningConfig) -> nn.Module:
        ...
        if target_sparsity > self.current_sparsity:
            ...
            logger.info(f"Gradual pruning step {self.pruning_step}: sparsity = {target_sparsity:.4f}")
    ...
    def _calculate_target_sparsity(self, config: PruningConfig) -> float:
        ...
        if config.pruning_schedule == "linear":
            ...
            return config.initial_sparsity + progress * (config.final_sparsity - config.initial_sparsity)
        elif config.pruning_schedule == "polynomial":
            progress = self.pruning_step / config.iterative_steps
            return config.initial_sparsity + (config.final_sparsity - config.initial_sparsity) * (progress ** 3)
        else:
            # Exponential
            progress = self.pruning_step / config.iterative_steps
            return config.initial_sparsity + (config.final_sparsity - config.initial_sparsity) * (1 - np.exp(-3 * progress))
...
class ModelPruner:
    ...
    def prune_and_fine_tune(self, model: nn.Module, X: pd.DataFrame, y: pd.Series) -> PruningResult:
        """
        ...
        try:
            ...
            logger.info(f"Original model - AUC: {original_performance.get('roc_auc', 0.0):.4f}, "
            ...
            if self.config.pruning_method == "gradual":
                pruned_model = self._gradual_prune_and_fine_tune(model, X_train, y_train, X_val, y_val)
            else:
                # Apply pruning
                ...
                if self.config.fine_tune_epochs > 0:
                    pruned_model = self._fine_tune_model(pruned_model, X_train, y_train, X_val, y_val)
            ...
            # Calculate metrics
            compression_ratio = original_stats['params'] / pruned_stats['params'] if pruned_stats['params'] > 0 else 1.0
            ...
            if not success:
                logger.warning(f"Pruning failed: AUC drop ({auc_drop:.4f}) exceeds threshold ({self.config.accuracy_threshold})")
            ...
            return PruningResult(
                ...
                flops_reduction=1.0 - (pruned_stats.get('flops', 0) / original_stats.get('flops', 1)),
                original_memory_mb=original_stats.get('memory_mb', 0.0),
                pruned_memory_mb=pruned_stats.get('memory_mb', 0.0),
                memory_reduction=1.0 - (pruned_stats.get('memory_mb', 0.0) / original_stats.get('memory_mb', 1.0)),
                ...
                message="Pruning completed successfully" if success else f"Pruning failed: AUC drop too large ({auc_drop:.4f})"
            )
    ...
    def _gradual_prune_and_fine_tune(self, model: nn.Module, X_train: pd.DataFrame, y_train: pd.Series,
        ...
        for step in range(self.config.iterative_steps):
            logger.info(f"Gradual pruning step {step + 1}/{self.config.iterative_steps}")
            ...
            if self.config.iterative_recovery:
                current_model = self._fine_tune_model_tensors(
                    current_model, X_train_tensor, y_train_tensor, 
                    X_val_tensor, y_val_tensor, self.config.recovery_epochs
                )
        
        return current_model
    
    def _fine_tune_model(self, model: nn.Module, X_train: pd.DataFrame, y_train: pd.Series,
                        X_val: pd.DataFrame, y_val: pd.Series) -> nn.Module:
        """Fine-tune pruned model."""
        # Convert to tensors
        X_train_tensor = torch.FloatTensor(X_train.values)
        ...
        return self._fine_tune_model_tensors(
            ...
            X_val_tensor, y_val_tensor, self.config.fine_tune_epochs
        )
    
    def _fine_tune_model_tensors(self, model: nn.Module, X_train: torch.Tensor, y_train: torch.Tensor,
                               X_val: torch.Tensor, y_val: torch.Tensor, epochs: int) -> nn.Module:
    ...
    def _evaluate_model(self, model: nn.Module, X_val: pd.DataFrame, y_val: pd.Series) -> Dict[str, float]:
    ...
    def _calculate_model_stats(self, model: nn.Module) -> Dict[str, Any]:
        ...
        for module in model.modules():
            if isinstance(module, nn.Linear):
                ...
                flops += module.in_channels * module.out_channels * np.prod(module.kernel_size)
...
# Utility functions
def prune_model(model: nn.Module, X: pd.DataFrame, y: pd.Series, 
                config: Optional[PruningConfig] = None) -> PruningResult:
    """
...
def analyze_pruning_impact(original_model: nn.Module, pruned_model: nn.Module,
    ...
    # Compression metrics
    compression_ratio = original_stats['params'] / pruned_stats['params'] if pruned_stats['params'] > 0 else 1.0
    memory_reduction = 1.0 - (pruned_stats['memory_mb'] / original_stats['memory_mb']) if original_stats['memory_mb'] > 0 else 0.0
    ...
    analysis = {
        'compression_metrics': {
            'compression_ratio': compression_ratio,
            'sparsity': sparsity,
            'parameter_reduction': 1.0 - (pruned_stats['params'] / original_stats['params']),
...
def load_pruned_model(model_path: str, model_class: type) -> Tuple[nn.Module, Dict[str, Any]]:
    """
...
def compare_pruning_methods(model: nn.Module, X: pd.DataFrame, y: pd.Series,
                           sparsity_levels: List[float] = [0.3, 0.5, 0.7, 0.9]) -> Dict[str, List[PruningResult]]:
    """
    ...
    for sparsity in sparsity_levels:
        ...
        # Structured pruning
        struct_config = get_structured_pruning_config(sparsity * 0.5)  # Lower ratio for structured
...
# Helper functions
def _evaluate_model_simple(model: nn.Module, X: pd.DataFrame, y: pd.Series) -> Dict[str, float]:
