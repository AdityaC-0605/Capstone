...
def create_student_model(
    ...
    if isinstance(teacher_model, nn.Sequential):
        ...
        logger.warning(
            "Custom student model creation not implemented for this architecture"
        )
        return copy.deepcopy(teacher_model)


def analyze_distillation_impact(
    teacher_model: nn.Module,
    student_model: nn.Module,
    X_test: pd.DataFrame,
    y_test: pd.Series,
    ...
    teacher_size = sum(
        p.numel() * p.element_size() for p in teacher_model.parameters()
    ) / (1024 * 1024)
    student_size = sum(
        p.numel() * p.element_size() for p in student_model.parameters()
    ) / (1024 * 1024)
    ...
    knowledge_retention = {
        key: (
            (student_perf.get(key, 0.0) / teacher_perf[key])
            if teacher_perf[key] > 0
            else 0.0
        )
        for key in teacher_perf.keys()
    }
    ...
    analysis = {
        ...
        "distillation_efficiency": {
            ...
            "compression_vs_performance": compression_ratio
            / (1.0 + performance_drop.get("roc_auc", 0.0)),
        },
    }
...
def compare_distillation_temperatures(
    teacher_model: nn.Module,
    student_model: nn.Module,
    X: pd.DataFrame,
    y: pd.Series,
    temperatures: List[float] = [2.0, 4.0, 6.0, 8.0],
) -> Dict[float, DistillationResult]:
    """
