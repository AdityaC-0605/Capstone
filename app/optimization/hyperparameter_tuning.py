...
class OptimizationConfig:
    ...
    objectives: List[str] = field(
        default_factory=lambda: ["accuracy", "energy_efficiency"]
    )
...
class HyperparameterOptimizer:
    ...
    def _setup_study(self):
        ...
        if self.config.use_multi_objective:
            directions = [
                "maximize" if obj == "accuracy" else "minimize"
    ...
    def _dnn_objective(
        self, trial, X: pd.DataFrame, y: pd.Series
    ...
    def _gnn_objective(
        ...
        config = GNNConfig(
            ...
            conv_type=trial.suggest_categorical(
                "conv_type", ["gcn", "gat", "graph_conv"]
            ),
            num_heads=(
                trial.suggest_int("num_heads", 2, 8)
                if trial.params.get("conv_type") == "gat"
                else 4
            ),
            dropout_rate=trial.suggest_float("dropout_rate", 0.1, 0.5),
    ...
    def _evaluate_config(
        self, trial, trainer_class, config, X: pd.DataFrame, y: pd.Series
    ) -> Union[float, List[float]]:
        ...
        try:
            ...
            if self.config.use_multi_objective:
                ...
                combined_score = (
                    accuracy * (1 - self.config.energy_weight)
    ...
    def _run_optimization(
        self, objective: Callable, model_type: str, config_class: type
    ) -> OptimizationResult:
        ...
        try:
            ...
            logger.info(
                f"{model_type.upper()} optimization completed in {optimization_time:.2f} seconds"
            )
    ...
    def _create_config_from_params(
        self, config_class: type, params: Dict[str, Any]
    ) -> Any:
        ...
        if config_class == DNNConfig:
            ...
            hidden_layers = [
                params.get(f"hidden_layer_{i}", 64) for i in range(n_layers)
            ]
...
def optimize_all_models(
    X: pd.DataFrame, y: pd.Series, config: Optional[OptimizationConfig] = None
