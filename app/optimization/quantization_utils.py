...
# Utility functions
def quantize_model(model: nn.Module, X: pd.DataFrame, y: pd.Series, 
...
def get_dynamic_quantization_config() -> QuantizationConfig:
    ...
    return QuantizationConfig(
        quantization_method="post_training_dynamic",
        validate_quantization=True
    )
...
def analyze_quantization_impact(original_model: nn.Module, quantized_model: nn.Module,
    ...
    quantized_perf = _evaluate_model_simple(quantized_model, X_test_tensor, y_test_tensor)
...
def compare_quantization_methods(model: nn.Module, X: pd.DataFrame, y: pd.Series) -> Dict[str, QuantizationResult]:
    """
...
def benchmark_quantized_model(model: nn.Module, X_test: pd.DataFrame, 
...
def validate_quantized_model_accuracy(original_model: nn.Module, quantized_model: nn.Module,
...
def _measure_inference_time_simple(model: nn.Module, X: torch.Tensor, num_runs: int = 100) -> float:
