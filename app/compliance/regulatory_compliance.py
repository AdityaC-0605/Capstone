...
class FCRAComplianceChecker:
    ...
    def check_permissible_purpose(self, context: Dict[str, Any]) -> Tuple[ComplianceStatus, Dict[str, Any]]:
    ...
    def check_adverse_action_notice(self, context: Dict[str, Any]) -> Tuple[ComplianceStatus, Dict[str, Any]]:
    ...
    def check_accuracy_procedures(self, context: Dict[str, Any]) -> Tuple[ComplianceStatus, Dict[str, Any]]:
    ...
    def check_dispute_procedures(self, context: Dict[str, Any]) -> Tuple[ComplianceStatus, Dict[str, Any]]:
...
class ECOAComplianceChecker:
    ...
    def check_prohibited_discrimination(self, context: Dict[str, Any]) -> Tuple[ComplianceStatus, Dict[str, Any]]:
        ...
        prohibited_used = [factor for factor in model_features 
    ...
    def check_adverse_action_reasons(self, context: Dict[str, Any]) -> Tuple[ComplianceStatus, Dict[str, Any]]:
    ...
    def check_data_collection(self, context: Dict[str, Any]) -> Tuple[ComplianceStatus, Dict[str, Any]]:
    ...
    def check_record_retention(self, context: Dict[str, Any]) -> Tuple[ComplianceStatus, Dict[str, Any]]:
...
class GDPRComplianceChecker:
    ...
    def check_lawful_basis(self, context: Dict[str, Any]) -> Tuple[ComplianceStatus, Dict[str, Any]]:
    ...
    def check_data_minimization(self, context: Dict[str, Any]) -> Tuple[ComplianceStatus, Dict[str, Any]]:
    ...
    def check_data_subject_rights(self, context: Dict[str, Any]) -> Tuple[ComplianceStatus, Dict[str, Any]]:
    ...
    def check_retention_limits(self, context: Dict[str, Any]) -> Tuple[ComplianceStatus, Dict[str, Any]]:
    ...
    def check_privacy_by_design(self, context: Dict[str, Any]) -> Tuple[ComplianceStatus, Dict[str, Any]]:
...
class AuditTrailManager:
    ...
    def create_data_processing_record(self, data_subject_id: str, processing_purpose: str,
                                    legal_basis: str, data_categories: List[str],
                                    retention_period: int, consent_given: bool = False) -> str:
    ...
    def get_audit_trail(self, start_date: Optional[datetime] = None,
                       ...
                       compliance_relevant_only: bool = False) -> List[AuditTrailEntry]:
    ...
    def get_data_processing_records(self, data_subject_id: Optional[str] = None) -> List[DataProcessingRecord]:
    ...
    def generate_audit_report(self, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        ...
        entries = self.get_audit_trail(start_date, end_date, compliance_relevant_only=True)
...
class RegulatoryComplianceValidator:
        ...
        for rule in checker.rules:
            ...
            try:
                ...
                if status == ComplianceStatus.NON_COMPLIANT:
                    ...
                    # Log violation
                    logger.warning(f"Compliance violation: {rule.rule_id} - {rule.title}")
    ...
    def validate_all_frameworks(self, context: Dict[str, Any]) -> Dict[ComplianceFramework, List[ComplianceViolation]]:
        """Validate compliance across all supported frameworks."""
        
        results = {}
        
        frameworks = [ComplianceFramework.FCRA, ComplianceFramework.ECOA, ComplianceFramework.GDPR]
    ...
    def get_compliance_status(self) -> Dict[str, Any]:
        ...
        for framework in ComplianceFramework:
            ...
            if not framework_violations:
                ...
                high_violations = [v for v in framework_violations if v.severity == ViolationSeverity.HIGH]
        ...
        return {
            "overall_status": "compliant" if not active_violations else "violations_present",
            "total_active_violations": len(active_violations),
            "total_resolved_violations": len([v for v in self.violations if v.resolved]),
            "framework_status": framework_status,
            "last_check": self.compliance_history[-1]["timestamp"] if self.compliance_history else None
        }
    
    def generate_compliance_report(self, include_resolved: bool = False) -> Dict[str, Any]:
        ...
        recent_history = self.compliance_history[-10:] if self.compliance_history else []
        
        return {
            "report_generated": datetime.now().isoformat(),
            "summary": {
                "total_violations": len(violations_to_include),
                "active_violations": len([v for v in violations_to_include if not v.resolved]),
                "resolved_violations": len([v for v in violations_to_include if v.resolved]),
            ...
            "recommendations": self._generate_compliance_recommendations(violations_to_include)
        }
    ...
    def _generate_compliance_recommendations(self, violations: List[ComplianceViolation]) -> List[str]:
        ...
        critical_violations = [v for v in violations if v.severity == ViolationSeverity.CRITICAL and not v.resolved]
        high_violations = [v for v in violations if v.severity == ViolationSeverity.HIGH and not v.resolved]
        ...
        fcra_violations = [v for v in violations if v.framework == ComplianceFramework.FCRA and not v.resolved]
        if fcra_violations:
            recommendations.append(
                "Review FCRA compliance procedures, particularly around permissible purposes and adverse action notices"
            )
        
        ecoa_violations = [v for v in violations if v.framework == ComplianceFramework.ECOA and not v.resolved]
        if ecoa_violations:
            recommendations.append(
                "Implement bias testing and mitigation to address ECOA discrimination concerns"
            )
        
        gdpr_violations = [v for v in violations if v.framework == ComplianceFramework.GDPR and not v.resolved]
        if gdpr_violations:
            recommendations.append(
        ...
        if not violations:
            recommendations.append("Maintain current compliance practices and continue regular monitoring")
...
def validate_credit_decision_compliance(decision_context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "overall_status": "compliant" if total_violations == 0 else "violations_detected"
    }
...
if __name__ == "__main__":
    ...
    context = {
        ...
        'bias_test_results': {
            ...
            'race': {'bias_detected': True, 'bias_level': 'moderate', 'metric_value': 0.12}
        },
