...
class FairnessMetricsCalculator:
    ...
    def calculate_metric(self, metric: FairnessMetric, y_true: np.ndarray, 
    ...
    def _demographic_parity(self, y_true: np.ndarray, y_pred: np.ndarray, 
                           y_prob: Optional[np.ndarray], protected_attr: np.ndarray) -> Dict[str, float]:
        """
    ...
    def _equal_opportunity(self, y_true: np.ndarray, y_pred: np.ndarray,
                          y_prob: Optional[np.ndarray], protected_attr: np.ndarray) -> Dict[str, float]:
        """
    ...
    def _equalized_odds(self, y_true: np.ndarray, y_pred: np.ndarray,
                       y_prob: Optional[np.ndarray], protected_attr: np.ndarray) -> Dict[str, float]:
        """
        ...
        if tpr_results:
            ...
            results['tpr_ratio'] = min(tpr_rates) / max(tpr_rates) if max(tpr_rates) > 0 else 0
        
        if fpr_results:
            fpr_rates = list(fpr_results.values())
            results['fpr_disparity'] = max(fpr_rates) - min(fpr_rates)
            results['fpr_ratio'] = min(fpr_rates) / max(fpr_rates) if max(fpr_rates) > 0 else 0
        
        # Overall equalized odds violation
        results['overall_violation'] = results.get('tpr_disparity', 0) + results.get('fpr_disparity', 0)
        
        return results
    
    def _calibration(self, y_true: np.ndarray, y_pred: np.ndarray,
                    y_prob: Optional[np.ndarray], protected_attr: np.ndarray) -> Dict[str, float]:
        """
    ...
    def _treatment_equality(self, y_true: np.ndarray, y_pred: np.ndarray,
                           y_prob: Optional[np.ndarray], protected_attr: np.ndarray) -> Dict[str, float]:
        """
    ...
    def _conditional_use_accuracy_equality(self, y_true: np.ndarray, y_pred: np.ndarray,
                                          y_prob: Optional[np.ndarray], protected_attr: np.ndarray) -> Dict[str, float]:
        """
        ...
        if ppv_results:
            ...
            results['ppv_ratio'] = min(ppv_values) / max(ppv_values) if max(ppv_values) > 0 else 0
        
        if npv_results:
            npv_values = list(npv_results.values())
            results['npv_disparity'] = max(npv_values) - min(npv_values)
            results['npv_ratio'] = min(npv_values) / max(npv_values) if max(npv_values) > 0 else 0
...
class ProtectedAttributeAnalyzer:
    ...
    def analyze_protected_groups(self, y_true: np.ndarray, y_pred: np.ndarray,
                                y_prob: Optional[np.ndarray], protected_attr: np.ndarray,
                                attribute_name: str) -> Dict[str, ProtectedGroupStats]:
    ...
    def _calculate_group_stats(self, group_name: str, y_true: np.ndarray,
                              y_pred: np.ndarray, y_prob: Optional[np.ndarray]) -> ProtectedGroupStats:
    ...
    def detect_representation_bias(self, protected_attr: np.ndarray,
        ...
        underrepresented_threshold = 10.0  # Less than 10% is considered underrepresented
        underrepresented_groups = [
            group for group, info in representation.items()
        ...
        return {
            ...
            'representation_bias_detected': disparity_ratio < 0.5  # Significant if ratio < 0.5
        }
...
class BiasDetector:
    ...
    def detect_bias(self, y_true: np.ndarray, y_pred: np.ndarray,
                   ...
                   metrics: Optional[List[FairnessMetric]] = None) -> List[BiasDetectionResult]:
        """
            ...
            for metric in metrics:
                try:
                    result = self._detect_bias_for_attribute_metric(
                        ...
                        protected_attr_enum, metric
                    )
                    results.append(result)
                    
                except Exception as e:
                    logger.error(f"Error detecting bias for {attr_name} - {metric.value}: {e}")
        
        # Store results in history
        self.detection_history.extend(results)
        
        # Log significant bias detections
        significant_bias = [r for r in results if r.bias_detected and r.bias_level in [BiasLevel.HIGH, BiasLevel.SEVERE]]
        if significant_bias:
            self._log_bias_violations(significant_bias)
        
        return results
    
    def _detect_bias_for_attribute_metric(self, y_true: np.ndarray, y_pred: np.ndarray,
                                         y_prob: Optional[np.ndarray], protected_attr: np.ndarray,
                                         attr_enum: ProtectedAttribute, metric: FairnessMetric) -> BiasDetectionResult:
        ...
        return BiasDetectionResult(
            ...
            details={
                ...
                'group_sizes': {str(g): int(np.sum(protected_attr == g)) for g in np.unique(protected_attr)}
            }
        )
    
    def _evaluate_bias_level(self, metric_values: Dict[str, float],
                            threshold_config: FairnessThreshold) -> Tuple[bool, BiasLevel, float]:
        """Evaluate bias level based on metric values and thresholds."""
        
        # Extract the main disparity metric
        disparity_keys = ['disparity', 'overall_violation', 'max_calibration_error', 'calibration_disparity']
        overall_metric = 0.0
    ...
    def _calculate_statistical_significance(self, y_true: np.ndarray, y_pred: np.ndarray,
                                           protected_attr: np.ndarray, metric: FairnessMetric) -> Optional[float]:
    ...
    def _permutation_test(self, y_true: np.ndarray, y_pred: np.ndarray,
                         protected_attr: np.ndarray, metric: FairnessMetric,
    ...
    def _log_bias_violations(self, violations: List[BiasDetectionResult]):
        ...
        for violation in violations:
            message = (f"Bias violation detected: {violation.metric.value} "
    ...
    def generate_bias_report(self, results: Optional[List[BiasDetectionResult]] = None) -> Dict[str, Any]:
        ...
        for metric, metric_results in by_metric.items():
            ...
            metric_violation_rates[metric] = len(metric_violations) / len(metric_results)
        ...
        return {
            ...
            "by_protected_attribute": {
                attr: {
                    ...
                    "worst_violation": max(attr_results, key=lambda x: x.overall_metric).bias_level.value
                }
                for attr, attr_results in by_attribute.items()
            },
            "by_fairness_metric": {
                metric: {
                    ...
                    "average_disparity": np.mean([r.overall_metric for r in metric_results])
                }
                for metric, metric_results in by_metric.items()
            },
    ...
    def _generate_recommendations(self, results: List[BiasDetectionResult]) -> List[str]:
        ...
        dp_violations = [r for r in results if r.metric == FairnessMetric.DEMOGRAPHIC_PARITY and r.bias_detected]
        if dp_violations:
            recommendations.append(
        ...
        eo_violations = [r for r in results if r.metric == FairnessMetric.EQUAL_OPPORTUNITY and r.bias_detected]
        if eo_violations:
            recommendations.append(
        ...
        underrepresented = []
        for result in results:
            small_groups = [g for g, size in result.details.get('group_sizes', {}).items() if size < 100]
        ...
        if not recommendations:
            recommendations.append("No significant bias detected. Continue monitoring fairness metrics.")
...
def create_bias_detector(custom_thresholds: Optional[List[FairnessThreshold]] = None) -> BiasDetector:
    """Create bias detector with optional custom thresholds."""
    return BiasDetector(custom_thresholds)


def analyze_dataset_bias(data: pd.DataFrame, target_column: str,
                        protected_columns: List[str]) -> Dict[str, Any]:
