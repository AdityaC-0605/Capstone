...
class LightGBMBenchmark:
    ...
    def run_benchmark(self, X: pd.DataFrame, y: pd.Series) -> List[BenchmarkResult]:
        ...
        logger.info(f"Starting LightGBM benchmark with {len(self.config.lightgbm_configs)} configurations")
    ...
    def _benchmark_single_config(self, X: pd.DataFrame, y: pd.Series, 
                                lgb_config: LightGBMConfig, config_name: str) -> BenchmarkResult:
        ...
        try:
            ...
            for run in range(self.config.n_runs):
                ...
                if trial_result.success:
                    result.accuracy_scores.append(trial_result.test_metrics.get('accuracy', 0.0))
                    result.precision_scores.append(trial_result.test_metrics.get('precision', 0.0))
                    result.recall_scores.append(trial_result.test_metrics.get('recall', 0.0))
                    result.f1_scores.append(trial_result.test_metrics.get('f1_score', 0.0))
                    result.auc_scores.append(trial_result.test_metrics.get('roc_auc', 0.0))
                    result.training_times.append(trial_result.training_time_seconds)
                    
                    # Measure inference time
                    inference_time = self._measure_inference_time(trial_result.model, X.head(100))
                    ...
                    if result.model_size_mb == 0.0:
                        result.model_size_mb = self._estimate_model_size(trial_result.model)
                        result.num_features = len(X.columns)
                        result.num_parameters = self._estimate_num_parameters(trial_result.model)
    ...
    def _run_single_trial(self, X: pd.DataFrame, y: pd.Series, 
                         lgb_config: LightGBMConfig, run_id: int) -> LightGBMResult:
    ...
    def _estimate_num_parameters(self, model) -> int:
        ...
        try:
            ...
            params_per_tree = model.config.num_leaves * 2  # Internal nodes + leaf values
    ...
    def generate_comparison_report(self) -> str:
        ...
        for result in self.results:
            ...
            report.append(f"  AUC-ROC: {result.mean_metrics.get('auc_roc', 0):.4f} ± {result.std_metrics.get('auc_roc', 0):.4f}")
            report.append(f"  F1-Score: {result.mean_metrics.get('f1_score', 0):.4f} ± {result.std_metrics.get('f1_score', 0):.4f}")
            report.append(f"  Training Time: {result.mean_metrics.get('training_time', 0):.2f}s ± {result.std_metrics.get('training_time', 0):.2f}s")
            report.append(f"  Inference Time: {result.mean_metrics.get('inference_time', 0):.2f}ms ± {result.std_metrics.get('inference_time', 0):.2f}ms")
            report.append(f"  Model Size: {result.model_size_mb:.2f} MB")
            report.append(f"  Parameters: {result.num_parameters:,}")
        
        # Best performing configuration
        best_auc_result = max(self.results, key=lambda r: r.mean_metrics.get('auc_roc', 0))
        fastest_result = min(self.results, key=lambda r: r.mean_metrics.get('training_time', float('inf')))
        smallest_result = min(self.results, key=lambda r: r.model_size_mb)
        
        report.append("\n" + "=" * 50)
        report.append("Best Configurations:")
        report.append(f"  Best AUC-ROC: {best_auc_result.config_name} ({best_auc_result.mean_metrics.get('auc_roc', 0):.4f})")
        report.append(f"  Fastest Training: {fastest_result.config_name} ({fastest_result.mean_metrics.get('training_time', 0):.2f}s)")
        report.append(f"  Smallest Model: {smallest_result.config_name} ({smallest_result.model_size_mb:.2f} MB)")
...
def run_lightgbm_benchmark(X: pd.DataFrame, y: pd.Series, 
                          config: Optional[BenchmarkConfig] = None) -> List[BenchmarkResult]:
