...
class DNNModel(BaseModel):
    ...
    def get_feature_importance(self) -> Dict[str, float]:
        ...
        dummy_input = torch.randn(
            1, self.input_dim, requires_grad=True, device=self.device
        )
...
class DNNTrainer:
    ...
    def _evaluate_model(
        self, model: DNNModel, X: pd.DataFrame, y: pd.Series, dataset_name: str
    ) -> Dict[str, float]:
...
def train_dnn_baseline(
    X: pd.DataFrame, y: pd.Series, config: Optional[DNNConfig] = None
) -> DNNResult:
