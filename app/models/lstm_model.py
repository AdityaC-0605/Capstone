...
class AttentionLayer(nn.Module):
    ...
    def forward(self, lstm_outputs: torch.Tensor, lengths: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Args:
        ...
        context = torch.sum(lstm_outputs * attention_weights.unsqueeze(-1), dim=1)  # (batch_size, hidden_size)
...
class LSTMModel(BaseModel):
    ...
    def predict(self, x: torch.Tensor, lengths: torch.Tensor, threshold: float = 0.5) -> torch.Tensor:
    ...
    def get_feature_importance(self) -> Dict[str, float]:
        ...
        if self.config.use_attention and self.last_attention_weights is not None:
            ...
            try:
                ...
                dummy_input = torch.randn(1, dummy_seq_len, self.config.input_size, 
            ...
            try:
                ...
                if self.feature_names:
                    ...
                    for i in range(self.config.input_size):
                        feature_importance[f"feature_{i}"] = 1.0 / self.config.input_size
...
class SequenceDataProcessor:
    ...
    def create_sequences(self, X: pd.DataFrame, y: pd.Series) -> Tuple[List[np.ndarray], List[int], np.ndarray]:
        """
    ...
    def pad_sequences(self, sequences: List[np.ndarray], max_length: Optional[int] = None) -> np.ndarray:
...
class LSTMTrainer:
    ...
    def train_and_evaluate(self, X: pd.DataFrame, y: pd.Series, 
                          test_size: float = 0.2) -> LSTMResult:
        ...
        try:
            ...
            logger.info(f"Created {len(sequences)} sequences with max length {max(lengths)}")
            
            # Split data
            seq_train, seq_test, len_train, len_test, y_train, y_test = train_test_split(
                sequences, lengths, targets, test_size=test_size, random_state=42, stratify=targets
            )
            
            seq_train, seq_val, len_train, len_val, y_train, y_val = train_test_split(
                seq_train, len_train, y_train, test_size=0.2, random_state=42, stratify=y_train
            )
            
            logger.info(f"Data split - Train: {len(seq_train)}, Val: {len(seq_val)}, Test: {len(seq_test)}")
    ...
    def _train_model(self, model: LSTMModel, seq_train: List[np.ndarray], len_train: List[int], y_train: np.ndarray,
                    seq_val: List[np.ndarray], len_val: List[int], y_val: np.ndarray) -> List[TrainingMetrics]:
        """Train the LSTM model."""
        
        # Prepare data
        X_train_padded = self.sequence_processor.pad_sequences(seq_train, self.config.max_sequence_length)
        X_val_padded = self.sequence_processor.pad_sequences(seq_val, self.config.max_sequence_length)
        ...
        X_train_scaled = model.scaler.transform(X_train_reshaped).reshape(n_samples, n_timesteps, n_features)
        
        n_val_samples = X_val_padded.shape[0]
        X_val_reshaped = X_val_padded.reshape(-1, n_features)
        X_val_scaled = model.scaler.transform(X_val_reshaped).reshape(n_val_samples, n_timesteps, n_features)
    ...
    def _evaluate_model(self, model: LSTMModel, sequences: List[np.ndarray], lengths: List[int], 
                       targets: np.ndarray, dataset_name: str) -> Dict[str, float]:
        """Evaluate model on a dataset."""
        try:
            # Prepare data
            X_padded = self.sequence_processor.pad_sequences(sequences, self.config.max_sequence_length)
            
            # Scale features
            n_samples, n_timesteps, n_features = X_padded.shape
            X_reshaped = X_padded.reshape(-1, n_features)
            X_scaled = model.scaler.transform(X_reshaped).reshape(n_samples, n_timesteps, n_features)
            ...
            metrics = {
                ...
                'precision': precision_score(targets, predictions_np, average='weighted', zero_division=0),
                'recall': recall_score(targets, predictions_np, average='weighted', zero_division=0),
                'f1_score': f1_score(targets, predictions_np, average='weighted', zero_division=0),
...
def train_lstm_baseline(X: pd.DataFrame, y: pd.Series, 
                       config: Optional[LSTMConfig] = None) -> LSTMResult:
