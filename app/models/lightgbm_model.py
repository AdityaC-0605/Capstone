...
class LightGBMOptimizer:
    ...
    def optimize(
        ...
        try:
            ...
            # Optimize
            logger.info(
                f"Starting hyperparameter optimization with {self.config.n_trials} trials"
            )
            self.study.optimize(
                objective,
            ...
            logger.info(
                f"Optimization completed. Best AUC: {self.study.best_value:.4f}"
            )
...
class LightGBMModel(BaseEstimator):
    ...
    def fit(
        ...
        try:
            ...
            logger.info(
                f"LightGBM training completed. Best iteration: {self.model.best_iteration}"
            )
    ...
    def get_feature_importance(
        self, importance_type: Optional[str] = None
    ) -> Dict[str, float]:
...
class LightGBMTrainer:
    ...
    def _perform_cross_validation(
        ...
        try:
            ...
            logger.info(
                f"CV Results - AUC: {cv_results['mean_scores'].get('roc_auc', 0):.4f} ± {cv_results['std_scores'].get('roc_auc', 0):.4f}"
            )
...
def train_lightgbm_baseline(
    X: pd.DataFrame, y: pd.Series, config: Optional[LightGBMConfig] = None
) -> LightGBMResult:
