...
class ConsentManager:
    ...
    def record_consent(self, data_subject_id: str, consent_type: ConsentType,
    ...
    def check_consent_validity(self, data_subject_id: str, purpose: ProcessingPurpose,
        ...
        for consent_record in self.consent_records.values():
            if (consent_record.data_subject_id == data_subject_id and
                ...
                data_category in consent_record.data_categories and
                consent_record.status == ConsentStatus.GIVEN and
                (consent_record.expires_at is None or consent_record.expires_at > current_time)):
                return True
            ...
            try:
                ...
                for consent_id, record_data in consent_data.items():
                    consent_record = ConsentRecord(
                        ...
                        given_at=datetime.fromisoformat(record_data['given_at']) if record_data.get('given_at') else None,
                        withdrawn_at=datetime.fromisoformat(record_data['withdrawn_at']) if record_data.get('withdrawn_at') else None,
...
class DataRetentionManager:
    ...
    def get_retention_policy(self, data_category: DataCategory, 
                           purpose: ProcessingPurpose) -> Optional[DataRetentionPolicy]:
    ...
    def check_data_expiration(self, data_records: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for record in data_records:
            ...
            if policy:
                expiration_date = created_at + timedelta(days=policy.retention_period_days)
        ...
        if policies_file.exists():
            try:
                ...
                for policy_id, policy_data in policies_data.items():
                    if policy_id not in self.retention_policies:  # Don't override defaults
...
class DataLineageTracker:
    ...
    def track_data_flow(self, data_subject_id: str, source_system: str,
                       destination_system: str, data_categories: Set[DataCategory],
                       processing_purpose: ProcessingPurpose, retention_policy_id: str,
                       consent_id: Optional[str] = None,
                       transformation_applied: Optional[str] = None) -> str:
    ...
    def get_system_lineage(self, system_name: str) -> List[DataLineageRecord]:
        ...
        return [
            record for record in self.lineage_records
            if record.source_system == system_name or record.destination_system == system_name
        ]
    ...
    def generate_lineage_report(self, data_subject_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'first_transfer': min(record.timestamp for record in subject_records).isoformat() if subject_records else None,
            'last_transfer': max(record.timestamp for record in subject_records).isoformat() if subject_records else None,
            'transfers': [
                {
            ...
            try:
                ...
                for record_data in lineage_data:
                    lineage_record = DataLineageRecord(
                        ...
                        transformation_applied=record_data.get('transformation_applied'),
...
class RightToBeForgottenManager:
    ...
    def submit_deletion_request(self, data_subject_id: str, 
                              verification_data: Optional[Dict[str, Any]] = None) -> str:
        ...
        deletion_request = DeletionRequest(
            ...
            legal_review_required=len(affected_systems) > 5  # Complex cases need review
        )
        ...
        logger.info(f"Deletion request submitted: {request_id} for subject {data_subject_id}")
        return request_id
    ...
    def process_deletion_request(self, request_id: str, 
        ...
        if approved:
            ...
            if all(deletion_results.values()):
                ...
                logger.warning(f"Partial deletion for request {request_id}: {deletion_results}")
        else:
    ...
    def _execute_deletion(self, deletion_request: DeletionRequest) -> Dict[str, bool]:
        ...
        for system in deletion_request.affected_systems:
            try:
                ...
                success = self._delete_from_system(system, deletion_request.data_subject_id)
                deletion_results[system] = success
                
                if success:
                    logger.info(f"Deleted data for {deletion_request.data_subject_id} from {system}")
                else:
                    logger.error(f"Failed to delete data for {deletion_request.data_subject_id} from {system}")
...
class GDPRComplianceManager:
    ...
    def _handle_access_request(self, data_subject_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'lineage_report': self.lineage_tracker.generate_lineage_report(data_subject_id)
        }
    ...
    def _handle_rectification_request(self, data_subject_id: str, **kwargs) -> Dict[str, Any]:
    ...
    def _handle_deletion_request(self, data_subject_id: str, **kwargs) -> Dict[str, Any]:
    ...
    def generate_compliance_report(self) -> Dict[str, Any]:
        ...
        pending_deletions = [
            req for req in self.deletion_manager.deletion_requests.values()
        ...
        return {
            ...
            'consent_summary': {
                ...
                'active_consents': len([
                    c for c in self.consent_manager.consent_records.values()
            ...
            'data_lineage_summary': {
                ...
                'unique_subjects': len(set(
                    record.data_subject_id for record in self.lineage_tracker.lineage_records
                ))
            },
