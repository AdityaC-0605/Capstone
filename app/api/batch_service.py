...
class BatchQueue:
    ...
    async def list_jobs(
        self, status: Optional[BatchJobStatus] = None
    ) -> List[BatchJob]:
...
class BatchProcessor:
    ...
    async def submit_batch_job(
        ...
        if len(applications) > self.config.max_batch_size:
            raise ValueError(
                f"Batch size {len(applications)} exceeds maximum {self.config.max_batch_size}"
            )
        ...
        logger.info(
            f"Batch job {job_id} submitted with {len(applications)} applications"
        )
        return job_id
    ...
    async def _process_job(self, job: BatchJob):
        ...
        logger.info(
            f"Processing batch job {job.job_id} with {job.total_applications} applications"
        )
        ...
        try:
                ...
                sustainability_context = (
                    self.sustainability_monitor.start_experiment_tracking(
                        f"batch_{job.job_id}",
                        {
                            ...
                            "batch_size": job.total_applications,
                        },
                    )
                )
            ...
            logger.info(
                f"Batch job {job.job_id} completed: {job.successful_predictions}/{job.total_applications} successful"
            )
